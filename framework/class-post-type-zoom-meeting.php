...
?>
		...
		{
			if ($this->is_archive()) {
				$text = esc_html__('Zoom Meetings and Webinars', 'edumall-child');
			}
		...
		public function register_sidebars()
		{
			...
			register_sidebar(array_merge($default_args, array(
				'id'          => self::SIDEBAR_ID,
				'name'        => esc_html__('Zoom Meeting Sidebar', 'edumall-child'),
				'description' => esc_html__('Add widgets here.', 'edumall-child'),
			)));
		}
		...
		public function shortcode_zoom_meeting($atts)
		{
			...
			if (empty($meeting_id)) {
				echo '<h4 class="no-meeting-id"><strong style="color:red;">' . esc_html__('ERROR: ', 'edumall-child') . '</strong>' . esc_html__('No meeting id set in the shortcode', 'edumall-child') . '</h4>';
			...
			if (isset($zoom_states[$meeting_id]['state']) && $zoom_states[$meeting_id]['state'] === "ended") {
				echo '<h3>' . esc_html__('This meeting has been ended by host.', 'edumall-child') . '</h3>';
...
<?php
				...
				if ($meeting) {
					...
					printf(esc_html__('Please try again ! Some error occured while trying to fetch meeting with id:  %d', 'edumall-child'), $meeting_id);
				}
			}
		...
		public function zoom_shortcode_join_link($zoom_meetings)
		{
			if (empty($zoom_meetings)) {
				echo "<p>" . esc_html__('Meeting is not defined. Try updating this meeting', 'edumall-child') . "</p>";

...
if (!class_exists('Edumall_Nav_Menu')) {
	class Edumall_Nav_Menu
	{
		...
		{
			delete_transient('edumall-nav-menus');
		}
		...
		{
			$results = get_transient('edumall-nav-menus');
			...
			$results[''] = esc_html__('Default Menu', 'edumall-child');

			set_transient('edumall-nav-menus', $results, HOUR_IN_SECONDS * 2);

			return $results;
		}
	}

	new Edumall_Nav_Menu();
}

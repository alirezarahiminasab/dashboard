	...
	class Edumall_Title_Bar
	{
		...
		public function get_list($default_option = false, $default_text = '')
		{
			$options = array(
				...
				'07'   => esc_html__('Style 07 (Overlay)', 'edumall-child'),
				'08'   => esc_html__('Style 08 (Search Form)', 'edumall-child'),
			);
		...
		public function render_title()
		{
			...
			if (Edumall_Portfolio::instance()->is_archive()) {
				...
				$title = sprintf(esc_html__('Archives: %s', 'edumall-child'), post_type_archive_title('', false));
			} elseif (is_home()) {
				...
				$title = Edumall::setting('title_bar_archive_year_title') . get_the_date(esc_html_x('Y', 'yearly archives date format', 'edumall-child'));
			} elseif (is_month()) {
				$title = Edumall::setting('title_bar_archive_month_title') . get_the_date(esc_html_x('F Y', 'monthly archives date format', 'edumall-child'));
			} elseif (is_day()) {
				$title = Edumall::setting('title_bar_archive_day_title') . get_the_date(esc_html_x('F j, Y', 'daily archives date format', 'edumall-child'));
			} elseif (is_search()) {

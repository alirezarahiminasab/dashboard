...
if (!class_exists('Edumall_Login_Register')) {
	...
	class Edumall_Login_Register
	{
		...
		public function login_scripts()
		{
			...
			$js_variables = array(
				'validatorMessages' => [
					'required'    => esc_html__('This field is required', 'edumall-child'),
					'remote'      => esc_html__('Please fix this field', 'edumall-child'),
					'email'       => esc_html__('A valid email address is required', 'edumall-child'),
					'url'         => esc_html__('Please enter a valid URL', 'edumall-child'),
					'date'        => esc_html__('Please enter a valid date', 'edumall-child'),
					'dateISO'     => esc_html__('Please enter a valid date (ISO)', 'edumall-child'),
					'number'      => esc_html__('Please enter a valid number.', 'edumall-child'),
					'digits'      => esc_html__('Please enter only digits.', 'edumall-child'),
					'creditcard'  => esc_html__('Please enter a valid credit card number', 'edumall-child'),
					'equalTo'     => esc_html__('Please enter the same value again', 'edumall-child'),
					'accept'      => esc_html__('Please enter a value with a valid extension', 'edumall-child'),
					'maxlength'   => esc_html__('Please enter no more than {0} characters', 'edumall-child'),
					'minlength'   => esc_html__('Please enter at least {0} characters', 'edumall-child'),
					'rangelength' => esc_html__('Please enter a value between {0} and {1} characters long', 'edumall-child'),
					'range'       => esc_html__('Please enter a value between {0} and {1}', 'edumall-child'),
					'max'         => esc_html__('Please enter a value less than or equal to {0}', 'edumall-child'),
					'min'         => esc_html__('Please enter a value greater than or equal to {0}', 'edumall-child'),
				],
			);
		...
		public function user_login()
		{
			...
			$success = false;
			$msg     = esc_html__('Username or password is wrong. Please try again!', 'edumall-child');
			...
			if ($user && wp_check_password($password, $user->data->user_pass, $user->ID)) {
				...
				if (!is_wp_error($user)) {
					$success = true;
					$msg     = esc_html__('Login successful, Redirecting...', 'edumall-child');
				}
			}
		...
		public function user_register()
		{
			...
			$email = filter_var($refer, FILTER_SANITIZE_EMAIL);

			$success = false;
			$msg     = esc_html__('Username/Email address is existing', 'edumall-child');

			if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
				$success = false;
				$msg     = esc_html__('A valid email address is required', 'edumall-child');
			} else {
				...
				if (!is_wp_error($user_id)) {
					...
					$msg     = esc_html__('Congratulations, register successful, Redirecting...', 'edumall-child');
					$success = true;
		...
		public function reset_password()
		{
			...
			if (empty($user_login)) {
				echo json_encode(array(
					'success'  => false,
					'messages' => esc_html__('Enter a username or email address.', 'edumall-child'),
				));
				wp_die();
			}
			...
			if (filter_var($user_login, FILTER_VALIDATE_EMAIL)) {
				...
				if (empty($user_data)) {
					echo json_encode(array(
						'success'  => false,
						'messages' => esc_html__('There is no user registered with that email address.', 'edumall-child'),
					));
					wp_die();
				}
			} else {
				...
				if (!$user_data) {
					echo json_encode(array(
						'success'  => false,
						'messages' => esc_html__('Invalid username', 'edumall-child'),
					));
					wp_die();
				}
			}
			...
			$message = esc_html__('Someone has requested a password reset for the following account:', 'edumall-child') . "\r\n\r\n";
			$message .= network_home_url('/') . "\r\n\r\n";
			$message .= sprintf(esc_html__('Username: %s', 'edumall-child'), $user_login) . "\r\n\r\n";
			$message .= esc_html__('If this was a mistake, just ignore this email and nothing will happen.', 'edumall-child') . "\r\n\r\n";
			$message .= esc_html__('To reset your password, visit the following address:', 'edumall-child') . "\r\n\r\n";
			...
			$title   = sprintf(esc_html__('[%s] Password Reset', 'edumall-child'), $blogname);
			$title   = apply_filters('retrieve_password_title', $title, $user_login, $user_data);
			$message = apply_filters('retrieve_password_message', $message, $key, $user_login, $user_data);
			if ($message && !wp_mail($user_email, wp_specialchars_decode($title), $message)) {
				echo json_encode(array(
					'success'  => false,
					'messages' => esc_html__('The email could not be sent.', 'edumall-child') . "<br />\n" . esc_html__('Possible reason: your host may have disabled the mail function.', 'edumall-child'),
				));
				wp_die();
			} else {
				echo json_encode(array(
					'success'  => true,
					'messages' => esc_html__('Please, Check your email to get new password', 'edumall-child'),
				));
				wp_die();
			}
		}
		...
		{
			echo do_shortcode('[miniorange_social_login shape="longbuttonwithtext" view="horizontal" appcnt="2" space="0" theme="default" height="44" color="000000" heading="' . esc_html__('or Log-in with', 'edumall-child') . '"]');
		}

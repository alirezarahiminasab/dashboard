	...
	class Edumall_Login_Register {
		...
		public function initialize() {
			...
			if ( defined( 'MO_OPENID_SOCIAL_LOGIN_VERSION' ) ) {
				...
				add_action( 'tutor_after_login_form', [ $this, 'add_social_login_buttons' ], 10 );
		...
		public function user_login() {
			...
			if ( ! $user instanceof WP_User || ! wp_check_password( $password, $user->data->user_pass, $user->ID ) ) {
				wp_send_json_error( [
					'messages' => esc_html__( 'Username or password is wrong. Please try again!', 'edumall' ),
				] );
			}
			...
			wp_send_json_success( [
				'messages'     => esc_html__( 'Login successful, Redirecting...', 'edumall' ),
		...
		public function user_register() {
			...
			if ( empty( $_POST['firstname'] ) || empty( $_POST['lastname'] ) || empty( $_POST['email'] ) || empty( $_POST['password'] ) || empty( $_POST['username'] ) ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'Please input all required fields.', 'edumall' ) ] );
			}
			...
			if ( ! is_email( $email ) ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'Please provide a valid email address.', 'edumall' ) ] );
			}

			if ( email_exists( $email ) ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'An account is already registered with your email address.', 'edumall' ) ] );
			}

			if ( ! validate_username( $user_login ) ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'Please enter a valid account username.', 'edumall' ) ] );
			}

			if ( username_exists( $user_login ) ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'An account is already registered with that username. Please choose another.', 'edumall' ) ] );
			}

			if ( ! isset( $_POST['accept_account'] ) && '1' !== $_POST['accept_account'] ) {
				wp_send_json_error( [ 'messages' => esc_html__( 'Please accept the Privacy Policy and Terms of Use.', 'edumall' ) ] );
			}
			 ...
			 * @var WP_Error   $errors
			 */
			$errors = apply_filters( 'edumall/user_register/errors', $errors, $user_login, $email );
			...
			if ( ! is_wp_error( $user_id ) ) {
				...
				wp_send_json_success( [ 'messages' => esc_html__( 'Congratulations, register successful, Redirecting...', 'edumall' ) ] );
			} else {
		...
		public function add_social_login_buttons() {
			?>
			<mo-openid-icons>
				<?php echo do_shortcode( '[miniorange_social_login shape="longbuttonwithtext" view="horizontal" appcnt="2" space="0" theme="default" height="44" color="000000" heading="' . esc_html__( 'or Log-in with', 'edumall' ) . '"]' ); ?>
			</mo-openid-icons>
			<script>

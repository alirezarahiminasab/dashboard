...
?>
	...
	public static function page_links()
	{
		wp_link_pages(array(
			...
			'link_before'      => '<span>',
			'link_after'       => '</span>',
			'nextpagelink'     => esc_html__('Next', 'edumall-child'),
			'previouspagelink' => esc_html__('Prev', 'edumall-child'),
		));
	}
	...
	public static function comment_navigation($args = array())
	{
		...
		if (get_comment_pages_count() > 1 && get_option('page_comments')) {
			...
			$args     = wp_parse_args($args, $defaults);
		?>
			<nav id="<?php echo esc_attr($args['container_id']); ?>" class="<?php echo esc_attr($args['container_class']); ?>">
				<h2 class="screen-reader-text"><?php esc_html_e('Comment navigation', 'edumall-child'); ?></h2>

				<div class="comment-nav-links">
					<?php paginate_comments_links(array(
						'prev_text' => esc_html__('Prev', 'edumall-child'),
						'next_text' => esc_html__('Next', 'edumall-child'),
						'type'      => 'list',
					)); ?>
				</div>
			</nav>
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
			<div id="comment-<?php comment_ID(); ?>" class="comment-wrap">
				<div class="comment-author vcard">
					<?php echo edumall_get_avatar($comment->user_id, $args['avatar_size']); ?>
				</div>
				<div class="comment-content">
					...
					<?php if ($comment->comment_approved == '0') : ?>
						<em class="comment-awaiting-messages"><?php esc_html_e('Your comment is awaiting moderation.', 'edumall-child') ?></em>
						<br />
					<?php endif; ?>
					...
					<div class="comment-footer">
						<div class="comment-datetime">
							<?php
							printf(esc_html__('%s at %s', 'edumall-child'), get_comment_date(), get_comment_time());
							?>
						</div>
						...
						<div class="comment-actions">
							<?php comment_reply_link(array_merge($args, array(
								...
								'reply_text' => esc_html__('Reply', 'edumall-child'),
							))); ?>
							<?php edit_comment_link('' . esc_html__('Edit', 'edumall-child')); ?>
						</div>
					</div>
				</div>
			</div>
	...
	public static function comment_form()
	{
		...
		$fields = array(
			'author' => '<div class="row"><div class="col-sm-6 comment-form-author"><input id="author" placeholder="' . esc_attr__('Your Name *', 'edumall-child') . '" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30" ' . $aria_req . '/></div>',
			'email'  => '<div class="col-sm-6 comment-form-email"><input id="email" placeholder="' . esc_attr__('Your Email *', 'edumall-child') . '" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" ' . $aria_req . '/></div></div>',
		);

		$comment_field = '<div class="row"><div class="col-md-12 comment-form-comment"><textarea id="comment" placeholder="' . esc_attr__('Your Comment', 'edumall-child') . '" name="comment" aria-required="true"></textarea></div></div>';

		$comments_args = array(
			'label_submit'        => esc_html__('Submit', 'edumall-child'),
			'title_reply'         => esc_html__('Leave your thought here', 'edumall-child'),
			'comment_notes_after' => '',
	...
	public static function post_author()
	{
			...
			<div class="entry-author">
				<div class="author-info">
					<div class="author-avatar">
						<?php echo edumall_get_avatar(get_the_author_meta('ID'), 100); ?>
		...
		public static function get_author_socials_html($user_id = false)
		{
			...
			?>
				<a class="<?php echo esc_attr($link_classes); ?>" aria-label="<?php echo esc_attr($social['label']); ?>" href="<?php echo esc_url($_social_url); ?>" target="_blank">
		...
		public static function get_user_socials_html($user_id = false)
		{
			...
			foreach ($socials as $social_name => $social) {
				...
				$link_classes = 'hint--bounce hint--top hint--primary';
			?>
				<a class="<?php echo esc_attr($link_classes); ?>" aria-label="<?php echo esc_attr($social['label']); ?>" href="<?php echo esc_url($_social_url); ?>" target="_blank" rel="nofollow">
					<i class="<?php echo esc_attr($social['icon_classes']); ?>"></i>
				</a>
			<?php
			}
		...
		public static function get_sharing_list($args = array())
		{
			...
			?>
							<a class="<?php echo esc_attr($link_classes . ' facebook'); ?>" target="<?php echo esc_attr($args['target']); ?>" aria-label="<?php esc_attr_e('Facebook', 'edumall-child'); ?>" href="<?php echo esc_url($facebook_url); ?>">
								<?php if ($args['style'] === 'text') : ?>
									<span><?php esc_html_e('Facebook', 'edumall-child'); ?></span>
								<?php else : ?>
						...
						?>
							<a class="<?php echo esc_attr($link_classes . ' twitter'); ?>" target="<?php echo esc_attr($args['target']); ?>" aria-label="<?php esc_attr_e('Twitter', 'edumall-child'); ?>" href="https://twitter.com/share?text=<?php echo rawurlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')); ?>&url=<?php echo rawurlencode(get_permalink()); ?>">
								<?php if ($args['style'] === 'text') : ?>
									<span><?php esc_html_e('Twitter', 'edumall-child'); ?></span>
								<?php else : ?>
						...
						?>
							<a class="<?php echo esc_attr($link_classes . ' tumblr'); ?>" target="<?php echo esc_attr($args['target']); ?>" aria-label="<?php esc_attr_e('Tumblr', 'edumall-child'); ?>" href="https://www.tumblr.com/share/link?url=<?php echo rawurlencode(get_permalink()); ?>&amp;name=<?php echo rawurlencode(get_the_title()); ?>">
								<?php if ($args['style'] === 'text') : ?>
									<span><?php esc_html_e('Tumblr', 'edumall-child'); ?></span>
								<?php else : ?>
						...
						?>
							<a class="<?php echo esc_attr($link_classes . ' linkedin'); ?>" target="<?php echo esc_attr($args['target']); ?>" aria-label="<?php esc_attr_e('Linkedin', 'edumall-child'); ?>" href="https://www.linkedin.com/shareArticle?mini=true&url=<?php echo rawurlencode(get_permalink()); ?>&amp;title=<?php echo rawurlencode(get_the_title()); ?>">
								<?php if ($args['style'] === 'text') : ?>
									<span><?php esc_html_e('Linkedin', 'edumall-child'); ?></span>
								<?php else : ?>
						...
						?>
							<a class="<?php echo esc_attr($link_classes . ' email'); ?>" target="<?php echo esc_attr($args['target']); ?>" aria-label="<?php esc_attr_e('Email', 'edumall-child'); ?>" href="mailto:?subject=<?php echo rawurlencode(get_the_title()); ?>&amp;body=<?php echo rawurlencode(get_permalink()); ?>">
								<?php if ($args['style'] === 'text') : ?>
									<span><?php esc_html_e('Email', 'edumall-child'); ?></span>
								<?php else : ?>
		...
		public static function social_icons($args = array())
		{
			...
			if (!empty($social_link)) {
				...
				foreach ($social_link as $key => $row_values) {
					?>
					<a class="<?php echo esc_attr($args['link_classes']); ?>" <?php if ($args['tooltip_enable']) : ?> aria-label="<?php echo esc_attr($row_values['tooltip']); ?>" <?php endif; ?> href="<?php echo esc_url($row_values['link_url']); ?>" <?php if ($social_link_target === '1') : ?> target="_blank" <?php endif; ?> rel="nofollow">
		...
		{
			echo '<img src="https://via.placeholder.com/' . $width . 'x' . $height . '?text=' . esc_attr__('No+Image', 'edumall-child') . '" alt="' . esc_attr__('Thumbnail', 'edumall-child') . '"/>';
		}

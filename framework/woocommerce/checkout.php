...
if (!class_exists('Edumall_Woo_Checkout')) {
	class Edumall_Woo_Checkout
	{
		...
		{
?>
			<h3 class="checkout-payment-info-heading"><?php esc_html_e('Payment information', 'edumall-child'); ?></h3>
<?php
		}
			...
			foreach ($fields['shipping'] as $field => $value) {
				...
				switch ($field) {
					...
					case 'shipping_address_2':
						...
						$fields['shipping'][$field]['label']   = esc_html__('Apt/Suite', 'edumall-child');
						break;
		...
		public function new_gateway_icon($icon, $id)
		{
			switch ($id) {
				...
				case 'triplea_payment_gateway':
					$icon = Edumall_Helper::get_file_contents(EDUMALL_THEME_SVG_DIR . '/payments/bitcoin.svg');
					break;

					// Default Icon for other gateways.
				default:
					$icon = Edumall_Helper::get_file_contents(EDUMALL_THEME_SVG_DIR . '/payments/card.svg');
					break;
			}

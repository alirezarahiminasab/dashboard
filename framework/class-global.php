...
if ( ! class_exists( 'Edumall_Global' ) ) {
	class Edumall_Global {
		...
		protected static $site_skin        = 'light';
		...
		function init_global_variable() {
			...
			if ( is_singular( 'portfolio' ) ) {
				$edumall_page_options = maybe_unserialize( get_post_meta( get_the_ID(), 'insight_portfolio_options', true ) );
			} elseif ( is_singular( 'post' ) ) {
				$edumall_page_options = maybe_unserialize( get_post_meta( get_the_ID(), 'insight_post_options', true ) );
			} elseif ( is_singular( 'page' ) ) {
				$edumall_page_options = maybe_unserialize( get_post_meta( get_the_ID(), 'insight_page_options', true ) );
			} elseif ( is_singular( 'product' ) ) {
				$edumall_page_options = maybe_unserialize( get_post_meta( get_the_ID(), 'insight_product_options', true ) );
			}
			if ( function_exists( 'is_shop' ) && is_shop() ) {
				// Get page id of shop.
				$page_id              = wc_get_page_id( 'shop' );
				$edumall_page_options = maybe_unserialize( get_post_meta( $page_id, 'insight_page_options', true ) );
			}
		}
		...
		function set_site_skin() {
			$site_skin = Edumall::setting( 'site_skin', 'light' );
			$site_skin = apply_filters( 'edumall_site_skin', $site_skin );

			self::$site_skin = $site_skin;
		}

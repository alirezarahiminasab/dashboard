...
if (!class_exists('Edumall_Actions_Filters')) {
	class Edumall_Actions_Filters
	{
		...
		public function initialize()
		{
			...
			add_filter('embed_oembed_html', array($this, 'add_wrapper_for_video'), 10, 4);
		...
		{
			$args['home_label']   = esc_html__('Home', 'edumall-child');
			$args['search_label'] = esc_html__('Search Result of &quot;%s&quot;', 'edumall-child');
			$args['404_label']    = esc_html__('404 Not Found', 'edumall-child');
		...
		public function add_extra_user_info($fields)
		{
			...
			$additional_fields = array(
				'phone_number' => array(
					'label' => esc_html__('Phone Number', 'edumall-child'),
					'icon'  => 'far fa-phone',
				),
			);
		 ...
		 * @return string
		 */
		public function add_wrapper_for_video($html, $url, $attr, $post_ID)
		{
			$array = array(
				'youtube.com',
				'wordpress.tv',
		...
		public function body_classes($classes)
		{
			...
			if (Edumall::is_dark_version()) {
				$classes[] = 'edumall-dark-scheme';
			} else {
				$classes[] = 'edumall-light-scheme';
			}

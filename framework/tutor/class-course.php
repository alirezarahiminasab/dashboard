				...
				if (!empty($get_announcements)) {
					foreach ($get_announcements as $announcement) {
						$announcements[] = [
							...
							'posted_by'   => sprintf(esc_html__('Posted by %s', 'edumall-child'), $announcement->display_name),
							'posted_date' => sprintf(esc_html__('%s ago', 'edumall-child'), human_time_diff(strtotime($announcement->post_date))),
						];
					}
				}

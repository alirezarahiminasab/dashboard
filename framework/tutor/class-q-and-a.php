...
if (!class_exists('Edumall_Q_And_A')) {
	class Edumall_Q_And_A
	{
		...
		public function qna_create_update()
		{
			...
			if (!$qna_text) {
				// Content validation.
				wp_send_json_error(array('message' => esc_html__('Empty Content Not Allowed!', 'edumall-child')));
			}
			...
			if (!empty($question_id)) {
				wp_send_json_success([
					'message' => esc_html__('Your answer submit successfully.', 'edumall-child'),
				]);
			} else {
				wp_send_json_success([
					'message' => esc_html__('Your question submit successfully.', 'edumall-child'),
				]);
			}
		}

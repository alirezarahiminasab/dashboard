...
$is_quick_view = apply_filters( 'edumall_content_quick_view', false );

$wrapper_classes = 'woo-single-gallery';

$main_image_size  = apply_filters( 'edumall_product_main_image_size', '570x658' );
$thumb_image_size = apply_filters( 'edumall_product_thumb_image_size', '126x145' );

if ( true === $is_quick_view ) {
	$feature_style = 'slider';
	$main_image_size = '470x540';
} else {
	$feature_style = Edumall_Woo::instance()->get_single_product_style();
}
...
<?php if ( ! empty( $attachment_ids ) ) {
	...
	<div class="<?php echo esc_attr( $classes ); ?>">
		<?php if ( $feature_style === 'single' ) { ?>
			...
			$main_image_html = Edumall_Image::get_attachment_by_id( array(
				'id'   => $attachment_id,
		...
		<?php } elseif ( $feature_style === 'slider' ) {
			...
			<div class="<?php echo esc_attr( $wrapper_classes ); ?>">
				...
				foreach ( $attachment_ids as $attachment_id ) {
					...
					$thumbs_image_html         = Edumall_Image::get_attachment_by_id( [
						'id'   => $attachment_id,
...
<?php } else {
	echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<img src="%s" alt="%s" />', wc_placeholder_img_src(), esc_attr__( 'Placeholder', 'edumall' ) ), $post->ID );
}
?>

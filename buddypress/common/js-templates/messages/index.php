...
<script type="text/html" id="tmpl-bp-messages-form">
	...
	<label for="send-to-input"><?php esc_html_e( 'Send @Username', 'edumall' ); ?></label>
	<input type="text" name="send_to" class="send-to-input" id="send-to-input" />

	<label for="subject"><?php _e( 'Subject', 'edumall' ); ?></label>
	...
	<div class="submit">
		<button type="submit" id="bp-messages-send" class="button bp-primary-action"><?php echo esc_html_x( 'Send', 'button', 'edumall' ); ?></button>
		<button type="reset" id="bp-messages-reset" class="text-button small bp-secondary-action"><?php echo esc_html_x( 'Reset', 'form reset button', 'edumall' ); ?></button>
	</div>
</script>
...
<script type="text/html" id="tmpl-bp-messages-paginate">
	<# if ( 1 !== data.page ) { #>
		<button id="bp-messages-prev-page"class="button messages-button">
			...
			<span class="bp-screen-reader-text"><?php echo esc_html_x( 'Previous page', 'link', 'edumall' ); ?></span>
		</button>
	<# } #>
	...
	<# if ( data.total_page !== data.page ) { #>
		<button id="bp-messages-next-page"class="button messages-button">
			...
			<span class="bp-screen-reader-text"><?php echo esc_html_x( 'Next page', 'link', 'edumall' ); ?></span>
		</button>
	<# } #>
</script>
...
<script type="text/html" id="tmpl-bp-bulk-actions">
	...
	<label for="user_messages_select_all"><?php esc_html_e( 'All Messages', 'edumall' ); ?></label>
	<div class="bulk-actions-wrap bp-hide">
		<div class="bulk-actions select-wrap">
			<label for="user-messages-bulk-actions" class="bp-screen-reader-text">
				<?php esc_html_e( 'Select bulk action', 'edumall' ); ?>
			</label>
		...
		<button class="messages-button bulk-apply bp-tooltip" type="submit" data-bp-tooltip="<?php echo esc_attr_x( 'Apply', 'button', 'edumall' ); ?>">
			<span class="dashicons dashicons-yes" aria-hidden="true"></span>
			<span class="bp-screen-reader-text"><?php echo esc_html_x( 'Apply', 'button', 'edumall' ); ?></span>
		</button>
	</div>
</script>
...
<script type="text/html" id="tmpl-bp-messages-thread">
	<div class="thread-cb">
		...
		<label for="bp-message-thread-{{data.id}}" class="bp-screen-reader-text"><?php esc_html_e( 'Select message:', 'edumall' ); ?> {{data.subject}}</label>
	</div>
	...
	<# if ( ! data.recipientsCount ) { #>
		<div class="thread-from">
			<a class="user-link" href="{{data.sender_link}}">
				...
				<span class="bp-screen-reader-text"><?php esc_html_e( 'From:', 'edumall' ); ?></span>
	...
	<# } else {
		...
		<div class="thread-to">
			<a class="user-link" href="{{recipient.user_link}}">
				...
				<span class="bp-screen-reader-text"><?php esc_html_e( 'To:', 'edumall' ); ?></span>
...
<script type="text/html" id="tmpl-bp-messages-preview">
	<# if ( undefined !== data.content ) { #>
		...
		<h2 class="message-title preview-thread-title"><?php esc_html_e( 'Active conversation:', 'edumall' ); ?><span class="messages-title">{{{data.subject}}}</span></h2>
		<div class="preview-content">
			<header class="preview-pane-header">

				<# if ( undefined !== data.recipients ) { #>
					<dl class="thread-participants">
						<dt><?php esc_html_e( 'Participants:', 'edumall' ); ?></dt>
						<dd>
				...
				<div class="actions">
					...
					<button type="button" class="message-action-delete bp-tooltip bp-icons" data-bp-action="delete" data-bp-tooltip="<?php esc_attr_e( 'Delete conversation.', 'edumall' ); ?>">
						<span class="bp-screen-reader-text"><?php esc_html_e( 'Delete conversation.', 'edumall' ); ?></span>
					</button>

					<# if ( undefined !== data.star_link ) { #>

						<# if ( false !== data.is_starred ) { #>
							<a role="button" class="message-action-unstar bp-tooltip bp-icons" href="{{data.star_link}}" data-bp-action="unstar" aria-pressed="true" data-bp-tooltip="<?php esc_attr_e( 'Unstar Conversation', 'edumall' ); ?>">
								<span class="bp-screen-reader-text"><?php esc_html_e( 'Unstar Conversation', 'edumall' ); ?></span>
							</a>
						<# } else { #>
							<a role="button" class="message-action-star bp-tooltip bp-icons" href="{{data.star_link}}" data-bp-action="star" aria-pressed="false" data-bp-tooltip="<?php esc_attr_e( 'Star Conversation', 'edumall' ); ?>">
								<span class="bp-screen-reader-text"><?php esc_html_e( 'Star Conversation', 'edumall' ); ?></span>
							</a>
						<# } #>

					<# } #>

					<a href="../view/{{data.id}}/" class="message-action-view bp-tooltip bp-icons" data-bp-action="view" data-bp-tooltip="<?php esc_attr_e( 'View full conversation and reply.', 'edumall' ); ?>">
						<span class="bp-screen-reader-text"><?php esc_html_e( 'View full conversation and reply.', 'edumall' ); ?></span>
					</a>
	...
	<div class="message-metadata">
		...
		<div class="actions">
			<# if ( undefined !== data.star_link ) { #>
				...
				<button type="button" class="message-action-unstar bp-tooltip bp-icons <# if ( false === data.is_starred ) { #>bp-hide<# } #>" data-bp-star-link="{{data.star_link}}" data-bp-action="unstar" data-bp-tooltip="<?php esc_attr_e( 'Unstar Message', 'edumall' ); ?>">
					<span class="bp-screen-reader-text"><?php esc_html_e( 'Unstar Message', 'edumall' ); ?></span>
				</button>

				<button type="button" class="message-action-star bp-tooltip bp-icons <# if ( false !== data.is_starred ) { #>bp-hide<# } #>" data-bp-star-link="{{data.star_link}}" data-bp-action="star" data-bp-tooltip="<?php esc_attr_e( 'Star Message', 'edumall' ); ?>">
					<span class="bp-screen-reader-text"><?php esc_html_e( 'Star Message', 'edumall' ); ?></span>
				</button>
		...
		<div class="message-box">
			<div class="message-metadata">
				...
				<div class="avatar-box">
					...
					<strong><?php esc_html_e( 'Send a Reply', 'edumall' ); ?></strong>
				</div>
			...
			<div class="message-content">
				...
				<label for="message_content" class="bp-screen-reader-text"><?php esc_html_e( 'Reply to Message', 'edumall' ); ?></label>
				<div id="bp-message-content"></div>

				<?php bp_nouveau_messages_hook( 'after', 'reply_box' ); ?>

				<div class="submit">
					<input type="submit" name="send" value="<?php echo esc_attr_x( 'Send Reply', 'button', 'edumall' ); ?>" id="send_reply_button"/>
				</div>

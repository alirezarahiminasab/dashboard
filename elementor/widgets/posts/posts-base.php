...
abstract class Posts_Base extends Base
{
	...
	protected function register_query_section()
	{
		$this->start_controls_section('query_section', [
			'label' => esc_html__('Query', 'edumall-child'),
		]);

		$this->add_control('query_source', [
			'label'   => esc_html__('Source', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => array(
				'custom_query'  => esc_html__('Custom Query', 'edumall-child'),
				'current_query' => esc_html__('Current Query', 'edumall-child'),
			),
		...
		$this->start_controls_tab('query_include_tab', [
			'label' => esc_html__('Include', 'edumall-child'),
		]);

		$this->add_control('query_include', [
			'label'       => esc_html__('Include By', 'edumall-child'),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT2,
			'multiple'    => true,
			'options'     => [
				'terms'   => esc_html__('Term', 'edumall-child'),
				'authors' => esc_html__('Author', 'edumall-child'),
			],
			'condition'   => [
				'query_source!' => ['current_query'],
			],
		...
		$this->add_control('query_include_authors', [
			'label'        => esc_html__('Author', 'edumall-child'),
			'label_block'  => true,
			'type'         => Module_Query_Base::AUTOCOMPLETE_CONTROL_ID,
			'multiple'     => true,
			'default'      => [],
		...
		$this->start_controls_tab('query_exclude_tab', [
			'label' => esc_html__('Exclude', 'edumall-child'),
		]);

		$this->add_control('query_exclude', [
			'label'       => esc_html__('Exclude By', 'edumall-child'),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT2,
			'multiple'    => true,
			'options'     => [
		...
		$this->add_control('query_number', [
			'label'       => esc_html__('Items per page', 'edumall-child'),
			'description' => esc_html__('Number of items to show per page. Input "-1" to show all posts. Leave blank to use global setting.', 'edumall-child'),
		...
		$this->add_control('query_offset', [
			...
			'description' => esc_html__('Number of items to displace or pass over.', 'edumall-child'),
			'type'        => Controls_Manager::NUMBER,
			'min'         => 0,
			'max'         => 100,
			'step'        => 1,
		...
		$this->add_control('query_orderby', [
			'label'       => esc_html__('Order by', 'edumall-child'),
			'description' => esc_html__('Select order type. If "Meta value" or "Meta value Number" is chosen then meta key is required.', 'edumall-child'),
			'type'        => Controls_Manager::SELECT,
			'options'     => array(
				'date'           => esc_html__('Date', 'edumall-child'),
				'ID'             => esc_html__('Post ID', 'edumall-child'),
				'author'         => esc_html__('Author', 'edumall-child'),
				'title'          => esc_html__('Title', 'edumall-child'),
				'modified'       => esc_html__('Last modified date', 'edumall-child'),
				'parent'         => esc_html__('Post/page parent ID', 'edumall-child'),
				'comment_count'  => esc_html__('Number of comments', 'edumall-child'),
				'menu_order'     => esc_html__('Menu order/Page Order', 'edumall-child'),
				'meta_value'     => esc_html__('Meta value', 'edumall-child'),
				'meta_value_num' => esc_html__('Meta value number', 'edumall-child'),
				'rand'           => esc_html__('Random order', 'edumall-child'),
			),
			'default'     => 'date',
			'condition'   => [
				'query_source!' => ['current_query'],
		...
		$this->add_control('query_sort_meta_key', [
			'label'     => esc_html__('Meta key', 'edumall-child'),
		...
		$this->add_control('query_order', [
			'label'     => esc_html__('Sort order', 'edumall-child'),
			'type'      => Controls_Manager::SELECT,
			'options'   => array(
				'DESC' => esc_html__('Descending', 'edumall-child'),
				'ASC'  => esc_html__('Ascending', 'edumall-child'),
			),
			'default'   => 'DESC',
			'condition' => [
				'query_source!' => ['current_query'],
	...
	protected function add_pagination_section()
	{
		$this->start_controls_section('pagination_section', [
			'label' => esc_html__('Pagination', 'edumall-child'),
		]);
		...
		$this->add_control('pagination_type', [
			...
			'options' => array(
				...
				'numbers'       => esc_html__('Numbers', 'edumall-child'),
				'navigation'    => esc_html__('Navigation', 'edumall-child'),
				'load-more'     => esc_html__('Button', 'edumall-child'),
				'load-more-alt' => esc_html__('Custom Button', 'edumall-child'),
				'infinite'      => esc_html__('Infinite Scroll', 'edumall-child'),
			),
			'default' => '',
		]);

		$this->add_control('pagination_custom_button_id', [
			'label'       => esc_html__('Custom Button ID', 'edumall-child'),
			'description' => esc_html__('Input id of custom button to load more posts when click. For e.g: #product-load-more-btn', 'edumall-child'),
			'type'        => Controls_Manager::TEXT,
			'condition'   => [
	...
	protected function add_pagination_style_section()
	{
		...
		$this->add_control('pagination_loading_heading', [
			'label'     => esc_html__('Loading Icon', 'edumall-child'),
	...
	protected function add_filter_section()
	{
		$this->start_controls_section('filter_section', [
			'label' => esc_html__('Filter', 'edumall-child'),
		]);

		$this->add_control('filter_enable', [
			'label' => esc_html__('Show Filter', 'edumall-child'),
		...
		$this->add_control('filter_counter', [
			'label'        => esc_html__('Show Counter', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
			'condition'    => [
				'filter_style!' => '',
		...
		$this->add_control('filter_in_grid', [
			'label'        => esc_html__('In Grid', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
			'condition'    => [
				'filter_style!' => '',
	...
	protected function add_filter_style_section()
	{
		...
		$this->add_group_control(Group_Control_Typography::get_type(), [
			...
			'label'    => esc_html__('Link Typography', 'edumall-child'),
		...
		$this->add_control('filter_counter_style_heading', [
			'label'     => esc_html__('Filter Counter', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'filter_counter' => '1',
	...
	protected function add_sorting_section()
	{
		$this->start_controls_section('result_count_sorting_section', [
			'label' => esc_html__('Result Count & Sorting', 'edumall-child'),
		]);

		$this->add_control('show_result_count', [
			'label'        => esc_html__('Show Result Count', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);

		$this->add_control('show_ordering', [
			'label'        => esc_html__('Show Order', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);
	...
	protected function get_sort_options()
	{
		return [
			...
			'date'       => esc_html__('Latest', 'edumall-child'),
		...
		<div class="edumall-grid-sorting row row-sm-center">
			<div class="col-md-6 result-count">
				<?php if (!empty($settings['show_result_count'])) : ?>
					...
					$result_count_html = sprintf(_n('%s course', '%s courses', $result_count, 'edumall-child'), '<span class="count">' . number_format_i18n($result_count) . '</span>');
					printf(
						wp_kses(
							__('We found %s available for you', 'edumall-child'),
							array('span' => ['class' => []])
						),
						$result_count_html
					);
			...
			<div class="col-md-6 ordering">
				<?php if (!empty($settings['show_ordering'])) : ?>
					...
					$selected        = '';
					$select_settings = [
						'fieldLabel' => esc_html__('Sort by:', 'edumall-child'),
					];
	...
	protected function print_pagination($query, $settings)
	{
		...
		?>
						...
						<div class="inner">
							<?php if ($pagination_type === 'load-more') { ?>
								<a href="#" class="edumall-load-more-button tm-button style-border icon-right">
									<span class="button-text"><?php echo esc_html__('Load More', 'edumall-child'); ?></span>
			...
			<div class="edumall-grid-messages" style="display: none;">
				<?php esc_html_e('All items displayed.', 'edumall-child'); ?>
			</div>
		<?php
		}
	}
	...
	protected function print_pagination_type_navigation()
	{
		...
		<div class="navigation-buttons">
			<div class="nav-link prev-link disabled" data-action="prev">
				<?php esc_html_e('Prev Projects', 'edumall-child'); ?>
			</div>
			<div class="nav-line"></div>
			<div class="nav-link next-link" data-action="next">
				<?php esc_html_e('Next Projects', 'edumall-child'); ?>
			</div>
		</div>
	<?php
	}
		...
		<div <?php $this->print_render_attribute_string('filter') ?>>
			...
			<div class="edumall-grid-filter-buttons">
				<a href="<?php echo esc_url(get_post_type_archive_link($post_type)); ?>" class="<?php echo esc_attr($btn_filter_all_class); ?>" data-filter="*" data-filter-count="<?php echo esc_attr($total); ?>">
					<span class="filter-text"><?php esc_html_e('All', 'edumall-child'); ?></span>
				</a>
				<?php
				if ($list === '') {

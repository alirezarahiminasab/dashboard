...
class Widget_Event extends Posts_Base
{
	...
	{
		return esc_html__('Event Grid', 'edumall-child');
	}
	...
	protected function add_grid_options_section()
	{
		...
		$this->add_responsive_control('grid_column_gutter', [
			'label'     => esc_html__('Column Gutter', 'edumall-child'),
			'type'      => Controls_Manager::NUMBER,
			'min'       => 0,
			'max'       => 200,
			'step'      => 1,
		...
		$this->add_responsive_control('grid_row_gutter', [
			'label'     => esc_html__('Row Gutter', 'edumall-child'),
			'type'      => Controls_Manager::NUMBER,
			'min'       => 0,
			'max'       => 200,
			'step'      => 1,
		...
		$this->add_responsive_control('grid_content_position', [
			'label'                => esc_html__('Content Position', 'edumall-child'),
			'type'                 => Controls_Manager::SELECT,
			'default'              => '',
			'options'              => [
				''       => esc_html__('Default', 'edumall-child'),
				'top'    => esc_html__('Top', 'edumall-child'),
				'middle' => esc_html__('Middle', 'edumall-child'),
		...
		$this->add_responsive_control('grid_content_alignment', [
			...
			'options'              => [
				...
				'center' => esc_html__('Center', 'edumall-child'),
	...
	private function add_content_section()
	{
		...
		$this->add_control('layout', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'minimal'           => esc_html__('Minimal', 'edumall-child'),
				'one-left-featured' => esc_html__('1 Left Featured', 'edumall-child'),
				'alternate-grid'    => esc_html__('Grid Alternate (2 Columns)', 'edumall-child'),
			],
			'default' => 'minimal',
		]);
	...
	private function add_style_section()
	{
		...
		$this->add_group_control(Group_Control_Typography::get_type(), [
			...
			'label'    => esc_html__('Day Typography', 'edumall-child'),
			'selector' => '{{WRAPPER}} .event-date .event-date--day',
		]);

		$this->add_control('location_style_heading', [
			'label'     => esc_html__('Location', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		]);

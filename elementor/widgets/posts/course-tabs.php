...
class Widget_Course_Tabs extends Base
{
	...
	{
		return esc_html__('Course Tabs', 'edumall-child');
	}
	...
	private function add_layout_section()
	{
		...
		$this->add_control('layout', [
			...
			'options' => [
				...
				'carousel' => esc_html__('Carousel', 'edumall-child'),
			],
			'default' => 'grid',
		]);
		...
		$this->add_control('title_collapse', [
			'label'        => esc_html__('Title Collapse', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'default'      => 'yes',
			'return_value' => 'yes',
			'separator'    => 'before',
	...
	protected function add_course_tabs_section()
	{
		$this->start_controls_section('course_tabs_section', [
			'label' => esc_html__('Course Tabs', 'edumall-child'),
		]);

		$this->add_control('number_posts', [
			'label'       => esc_html__('Number posts', 'edumall-child'),
			'description' => esc_html__('Select number of courses display on each of tab.', 'edumall-child'),
		...
		$source_options = [
			'latest'      => esc_html__('All Courses (Latest)', 'edumall-child'),
			'trending'    => esc_html__('Trending Courses', 'edumall-child'),
			'popular'     => esc_html__('Popular Courses', 'edumall-child'),
			'featured'    => esc_html__('Featured Courses', 'edumall-child'),
			'by_category' => esc_html__('By Category', 'edumall-child'),
		];
		...
		$repeater->add_control('title', [
			'label'       => esc_html__('Tab Title', 'edumall-child'),
			'type'        => Controls_Manager::TEXT,
			'description' => esc_html__('Leave blank to use default.', 'edumall-child'),
		]);
		...
		$this->add_control('tabs', [
			...
			'default'     => [
				[
					'title'        => esc_html__('All', 'edumall-child'),
					'query_source' => 'latest',
				],
			],
		]);
	...
	private function add_tabs_style_section()
	{
		...
		$this->add_group_control(Group_Control_Typography::get_type(), [
			...
			'label'    => esc_html__('Item Typography', 'edumall-child'),
		...
		$this->add_group_control(Group_Control_Text_Gradient::get_type(), [
			...
			'selector' => '{{WRAPPER}} .edumall-tabpanel > .edumall-nav-tabs li a:hover',
		]);
		...
		$this->add_group_control(Group_Control_Text_Gradient::get_type(), [
			...
			'selector' => '{{WRAPPER}} .edumall-tabpanel > .edumall-nav-tabs li.active a',
		]);
...
?>
	...
	private function get_tab_title($tab)
	{
		...
		switch ($tab['query_source']) {
			case 'latest':
				$text = esc_html__('All', 'edumall-child');
				break;
			case 'trending':
				$text = esc_html__('Trending', 'edumall-child');
				break;
			case 'popular':
				$text = esc_html__('Popularity', 'edumall-child');
				break;
			case 'featured':
				$text = esc_html__('Featured', 'edumall-child');
				break;
			case 'by_category':
	...
	?>
		<div class="edumall-skeleton-card style-01">
			...
			<div class="edumall-skeleton-item edumall-skeleton-description"></div>
		</div>
	<?php
	}
	...
	private function print_slider()
	{
		...
		$number_posts = $settings['number_posts'];
	?>
		<div class="tm-tab-course-element tm-swiper v-stretch bullets-v-align-below nav-style-01 pagination-style-01 edumall-courses style-carousel-02" data-lg-items="auto" data-lg-gutter="30" data-nav="1" data-loop="0">
		...
		<div class="edumall-grid-response-messages"></div>
	<?php
	}

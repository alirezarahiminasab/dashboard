...
class Widget_Icon_Box extends Base
{
	...
	{
		return esc_html__('Modern Icon Box', 'edumall-child');
	}
	...
	private function add_icon_box_section()
	{
		$this->start_controls_section('icon_box_section', [
			'label' => esc_html__('Icon Box', 'edumall-child'),
		]);
		...
		$this->add_control('style', [
			...
			'options'      => [
				...
				'04' => esc_html__('04', 'edumall-child'),
			],
			'default'      => '',
	...
	private function add_content_icon_section()
	{
		...
		$this->add_control('view', [
			...
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				'default' => esc_html__('Default', 'edumall-child'),
				'stacked' => esc_html__('Stacked', 'edumall-child'),
				'bubble'  => esc_html__('Bubble', 'edumall-child'),
			],
			'default'      => 'default',
		...
		$this->add_control('shape', [
			'label'        => esc_html__('Shape', 'edumall-child'),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				'circle' => esc_html__('Circle', 'edumall-child'),
				'square' => esc_html__('Square', 'edumall-child'),
			],
			'default'      => 'circle',
			'condition'    => [
				'view'         => ['stacked'],
		...
		$this->add_control('position', [
			'label'        => esc_html__('Position', 'edumall-child'),
	...
	private function add_content_title_section()
	{
		...
		$this->add_control('title_divider_enable', [
			'label' => esc_html__('Display Divider', 'edumall-child'),
	...
	private function add_content_description_section()
	{
		...
		$this->add_control('description_text', [
			...
			'default'     => esc_html__('Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis.', 'edumall-child'),
	...
	private function add_icon_svg_animate_section()
	{
		$this->start_controls_section('icon_svg_animate_section', [
			'label'     => esc_html__('Icon SVG Animate', 'edumall-child'),
			'condition' => [
				'icon[library]' => 'svg',
			],
		]);
		...
		$this->add_control('icon_svg_animate_alert', [
			...
			'raw'             => esc_html__('Note: Animate works only with Stroke SVG Icon.', 'edumall-child'),
			'separator'       => 'after',
		]);

		$this->add_control('icon_svg_animate', [
			'label' => esc_html__('SVG Animate', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('icon_svg_animate_play_on_hover', [
			'label' => esc_html__('Play on hover', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('icon_svg_animate_type', [
			'label'   => esc_html__('Type', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'delayed'  => esc_html__('Delayed', 'edumall-child'),
				'sync'     => esc_html__('Sync', 'edumall-child'),
				'oneByOne' => esc_html__('One By One', 'edumall-child'),
			],
			'default' => 'delayed',
		]);

		$this->add_control('icon_svg_animate_duration', [
			'label'   => esc_html__('Transition Duration', 'edumall-child'),
	...
	private function add_box_style_section()
	{
		...
		$this->add_control('box_line_heading', [
			'label'     => esc_html__('Special Line', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'style' => ['02'],
	...
	private function add_icon_style_section()
	{
		...
		$this->add_control('icon_duotone_color_1_hr', [
			'label'     => esc_html__('Color 1', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'icon[value]!'  => '',
		...
		$this->add_control('icon_duotone_color_2_hr', [
			'label'     => esc_html__('Color 2', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'icon[value]!'  => '',
		...
		$this->add_control('icon_view_heading', [
			'label'     => esc_html__('Icon View', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'view' => ['stacked', 'bubble'],
	...
	private function add_title_divider_style()
	{
		$this->start_controls_section('title_divider_style_section', [
			'label'     => esc_html__('Title Divider', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
...
?>
	...
	?>
		<# var svg_id='<?php echo esc_html($id); ?>' ; #>
			...
			<# view.addRenderAttribute( 'box' , 'class' , 'tm-icon-box edumall-box' ); var box_tag='div' ; if ( '' !==settings.link.url && 'box'===settings.link_click ) { box_tag='a' ; view.addRenderAttribute( 'box' , 'class' , 'link-secret' ); view.addRenderAttribute( 'box' , 'href' , '#' ); } view.addRenderAttribute( 'icon' , 'class' , 'edumall-icon icon' ); if ( 'svg'===settings.icon.library ) { view.addRenderAttribute( 'icon' , 'class' , 'edumall-svg-icon' ); } switch( settings.icon_color_type ) { case 'gradient' : view.addRenderAttribute( 'icon' , 'class' , 'edumall-gradient-icon' ); break; case 'classic' : view.addRenderAttribute( 'icon' , 'class' , 'edumall-solid-icon' ); break; } var iconHTML=elementor.helpers.renderIcon( view, settings.icon, { 'aria-hidden' : true }, 'i' , 'object' ); #>

...
class Modify_Widget_Section extends Modify_Base
{
	...
	public function section_layout($element)
	{
		...
		if (isset($gap_options['default'])) {
			$gap_options['default'] = esc_html__('Normal', 'edumall-child');
		}

		// Add new gap option 'custom'.
		if (!isset($gap_options['custom'])) {
			$gap_options['custom'] = esc_html__('Custom', 'edumall-child');
		}
		...
		$element->add_control('gap_beside', [
			'label'        => esc_html__('Gap Beside', 'edumall-child'),
			'type'         => Controls_Manager::SELECT,
			'default'      => 'yes',
			'options'      => [
				'no'  => esc_html__('No', 'edumall-child'),
				'yes' => esc_html__('Yes', 'edumall-child'),
			],
		...
		$element->add_responsive_control('gap_width', [
			'label'     => esc_html__('Gap Width', 'edumall-child'),
		...
		$element->add_control('column_vertical_alignment', [
			'label'                => esc_html__('Column Vertical Alignment', 'edumall-child'),
			'label_block'          => true,
			'type'                 => Controls_Manager::CHOOSE,

...
class Widget_Team_Member extends Base {
	...
	public function get_title() {
		return esc_html__( 'Team Member', 'edumall' );
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'social_networks', [
			...
			'default'     => [
				[
					'title' => esc_html__( 'Facebook', 'edumall' ),
					'link'  => [
						'url'         => '#',
						'is_external' => true,
						'nofollow'    => true,
				...
				[
					'title' => esc_html__( 'Twitter', 'edumall' ),
					'link'  => [
						'url'         => '#',
						'is_external' => true,
						'nofollow'    => true,
				...
				[
					'title' => esc_html__( 'Instagram', 'edumall' ),
					'link'  => [
						'url'         => '#',
						'is_external' => true,
						'nofollow'    => true,
				...
				[
					'title' => esc_html__( 'Youtube Channel', 'edumall' ),
					'link'  => [
						'url'         => '#',
						'is_external' => true,
						'nofollow'    => true,
	...
	private function add_content_style_section() {
		...
		$this->add_responsive_control( 'text_align', [
			...
			'default'              => '',
			'selectors'            => [
				'{{WRAPPER}} .tm-team-member.edumall-box' => 'text-align: {{VALUE}} !important;',
			],
		] );
		...
		$this->add_control( 'btn_bgr_hover', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .course-button a:after, {{WRAPPER}} .course-button a:before' => 'background-color: {{VALUE}} ;',
			],
			'condition' => [
				'style' => '04',
			],
					...
					<div class="info">
						...
						<?php if ( in_array( $settings['style'], array( '04' ) ) ) : ?>
							<?php if ( ! empty( $settings['rating'] ) ): ?>
								<?php echo \Edumall_Templates::render_rating( $settings['rating'], [ 'wrapper_class' => 'team-member-rating' ] ); ?>

								<?php if ( ! empty( $settings['total_rating'] ) ) : ?>
									<span class="rating member-rating">
										<?php echo '(' . esc_html( $settings['total_rating'] ) . ')'; ?>
									</span>
								<?php endif; ?>
							<?php endif; ?>

							<?php if ( ! empty( $settings['course'] ) ) : ?>
								<div class="course-button">
									<?php echo '<a class=" tm-button style-border" ' . $this->get_render_attribute_string( 'profile' ) . '>';
									echo '<span class="button-text">';
									echo esc_html( $settings['course'] );
									echo '</span>';
									echo '</a>'; ?>
								</div>
							<?php endif; ?>

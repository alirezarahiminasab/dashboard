...
class Widget_Product_Categories extends Base
{
	...
	{
		return esc_html__('Product Categories', 'edumall-child');
	}
	...
	private function add_query_section()
	{
		...
		$this->add_control('source', [
			...
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				''                      => esc_html__('Show All', 'edumall-child'),
				'by_id'                 => esc_html__('Manual Selection', 'edumall-child'),
				'by_parent'             => esc_html__('By Parent', 'edumall-child'),
				'current_subcategories' => esc_html__('Current Subcategories', 'edumall-child'),
			],
			'label_block' => true,
		]);
		...
		$parent_options = ['0' => esc_html__('Only Top Level', 'edumall-child')] + $options;
		$this->add_control(
			'parent',
			[
				'label'     => esc_html__('Parent', 'edumall-child'),
				'type'      => Controls_Manager::SELECT,
				'default'   => '0',
		...
		$this->add_control('hide_empty', [
			'label'     => esc_html__('Hide Empty', 'edumall-child'),
			'type'      => Controls_Manager::SWITCHER,
			'default'   => 'yes',
			'label_on'  => 'Hide',
			'label_off' => 'Show',
		...
		$this->add_control('number', [
			'label'   => esc_html__('Categories Count', 'edumall-child'),
			'type'    => Controls_Manager::NUMBER,
			'default' => '4',
		]);
		...
		$this->add_control('orderby', [
			...
			'default' => 'name',
			'options' => [
				'name'        => esc_html__('Name', 'edumall-child'),
				'slug'        => esc_html__('Slug', 'edumall-child'),
				'description' => esc_html__('Description', 'edumall-child'),
				'count'       => esc_html__('Count', 'edumall-child'),
			],
		]);

		$this->add_control('order', [
			'label'   => esc_html__('Order', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'default' => 'desc',
			'options' => [
				'asc'  => esc_html__('ASC', 'edumall-child'),
				'desc' => esc_html__('DESC', 'edumall-child'),
			],
		]);
	...
	private function add_image_style_section()
	{
		$this->start_controls_section('images_style_section', [
			'label' => esc_html__('Images', 'edumall-child'),
	...
	protected function render()
	{
		...
		$this->add_render_attribute('grid-wrapper', 'class', 'edumall-grid-wrapper edumall-product-categories');
...
?>
	...
	{
		$settings   = $this->get_settings_for_display();
		$image_size = \Edumall_Image::elementor_parse_image_size($settings, '600x600');
		...
		?>
			<div class="grid-item">
				<a href="<?php echo esc_url($link); ?>" class="product-cat-wrapper edumall-box cat-link link-secret">

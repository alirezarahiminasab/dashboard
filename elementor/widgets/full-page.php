...
class Widget_Full_Page extends Base
{
	...
	public function __construct($data = [], $args = null)
	{
		...
		wp_register_style('edumall-widget-fullpage', EDUMALL_THEME_URI . '/assets/libs/fullpage/fullpage.min.css', null, '3.0.5');

		// Fullpage.
		wp_register_script('fullpage', EDUMALL_THEME_URI . '/assets/libs/fullpage/fullpage.js', array(), null, true);
		//wp_register_script( 'fullpage-extensions', EDUMALL_THEME_URI . '/assets/libs/fullpage/fullpage.extensions.min.js', array(), null, true );
		wp_register_script('scrolloverflow', EDUMALL_THEME_URI . '/assets/libs/fullpage/vendors/scrolloverflow.js', array(), null, true);
		wp_register_script('edumall-widget-fullpage', EDUMALL_ELEMENTOR_URI . '/assets/js/widgets/widget-fullpage.js', array(
			'elementor-frontend',
			'fullpage',
			'scrolloverflow',
		), null, true);
	...
	{
		return esc_html__('Fullpage', 'edumall-child');
	}
	...
	{
		return ['edumall-widget-fullpage'];
	}
...
?>
	...
	private function add_slides_section()
	{
		$this->start_controls_section('slides_section', [
			'label' => esc_html__('Slides', 'edumall-child'),
		]);
		...
		$repeater->start_controls_tab('repeater_slides_content_tab', ['label' => esc_html__('Content', 'edumall-child')]);
		...
		$repeater->add_control('title', [
			...
			'default'     => esc_html__('Title', 'edumall-child'),
			'label_block' => true,
		]);

		$repeater->add_control('anchor', [
			'label' => esc_html__('Anchor', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);

		$repeater->add_control('skin', [
			'label'       => esc_html__('Skin', 'edumall-child'),
			'description' => esc_html__('Controls the skin of pagination & header when view this section.', 'edumall-child'),
			'type'        => Controls_Manager::CHOOSE,
			'options'     => [
				'light' => [
					'text' => esc_html__('Light', 'edumall-child'),
				],
				'dark'  => [
					'text' => esc_html__('Dark', 'edumall-child'),
				],
			],
			'toggle'      => false,
			'default'     => 'light',
		...
		$repeater->add_control('auto_height', [
			'label' => esc_html__('Auto Height', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$repeater->end_controls_tab();

		$repeater->start_controls_tab('repeater_slides_background_tab', ['label' => esc_html__('Background', 'edumall-child')]);
		...
		$this->add_control('slides', [
			...
			'default'     => [
				[
					'title' => esc_html__('Slide #1', 'edumall-child'),
				],
				[
					'title' => esc_html__('Slide #2', 'edumall-child'),
				],
			],
			'title_field' => '{{{ title }}}',
		]);
	...
	private function add_fullpage_options_section()
	{
		$this->start_controls_section('fullpage_options_section', [
			'label' => esc_html__('Fullpage Options', 'edumall-child'),
		]);

		$this->add_control('dots', [
			'label' => esc_html__('Dots Enable', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('dots_alignment', [
			'label'     => esc_html__('Dots Alignment', 'edumall-child'),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'left'  => [
		...
		$this->add_control('numbers', [
			'label' => esc_html__('Numbers Enable', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('sharing', [
			'label' => esc_html__('Share Enable', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);
	...
	?>
		<div id="full-page-sharing" class="full-page-share-list">
			<div class="stalk">
				<?php esc_html_e('Stalk', 'edumall-child'); ?>
			</div>
			<?php \Edumall_Templates::get_sharing_list(array(
	...
	?>
		<div id="full-page-numbers" class="full-page-numbers">
			...
			<div class="title">
				<?php esc_html_e('Projects', 'edumall-child'); ?>
			</div>
		</div>
<?php
	}

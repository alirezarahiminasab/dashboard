...
class Widget_Google_Map extends Base
{
	...
	{
		return esc_html__('Advanced Google Map', 'edumall-child');
	}
	...
	{
		return ['edumall-gmap3', 'edumall-maps', 'edumall-widget-google-map'];
	}
	...
	private function add_map_options_section()
	{
		$this->start_controls_section('map_options_section', [
			'label' => esc_html__('Map', 'edumall-child'),
		]);

		$this->add_responsive_control('height', [
			'label'   => esc_html__('Height', 'edumall-child'),
			...
			'selectors'  => [
				'{{WRAPPER}} .tm-google-map .map' => 'height: {{SIZE}}{{UNIT}};',
			],
		]);

		$this->add_control('style', [
			'label'   => esc_html__('Map Style', 'edumall-child'),
		...
		$this->add_control('type', [
			'label'   => esc_html__('Map Type', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__('Roadmap', 'edumall-child'),
				'satellite' => esc_html__('Satellite', 'edumall-child'),
				'hybrid'    => esc_html__('Hybrid', 'edumall-child'),
				'terrain'   => esc_html__('Terrain', 'edumall-child'),
			],
			'default' => 'roadmap',
		]);

		$this->add_control('zoom_level', [
			'label'       => esc_html__('Zoom Level', 'edumall-child'),
			'placeholder' => '12',
		...
		$this->add_control('scrollwheel', [
			'label'   => esc_html__('Mouse Scroll Wheel Zoom', 'edumall-child'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('draggable', [
			'label'   => esc_html__('Draggable', 'edumall-child'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('map_controls_heading', [
			'label'     => esc_html__('Controls', 'edumall-child'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		]);

		$this->add_control('zoom_control', [
			'label' => esc_html__('Zoom Control', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('scale_control', [
			'label' => esc_html__('Scale Control', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('fullscreen_control', [
			'label' => esc_html__('Fullscreen Control', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('street_view_control', [
			'label' => esc_html__('Street View Control', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);
	...
	private function add_map_markers_section()
	{
		$this->start_controls_section('markers_section', [
			'label' => esc_html__('Markers', 'edumall-child'),
		]);

		$this->add_control('marker_style', [
			'label'       => esc_html__('Marker Style', 'edumall-child'),
			'description' => esc_html__('Select default style for all markers', 'edumall-child'),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__('Icon', 'edumall-child'),
				'signal' => esc_html__('Signal', 'edumall-child'),
			],
			'default'     => 'icon',
		]);

		$this->add_control('marker_icon', [
			'label'     => esc_html__('Marker Icon', 'edumall-child'),
		...
		$repeater->start_controls_tab('marker_content_tab', [
			'label' => esc_html__('Marker', 'edumall-child'),
		]);

		$repeater->add_control('address', [
			'label'       => esc_html__('Address or Coordinate', 'edumall-child'),
		...
		$repeater->start_controls_tab('marker_popup_tab', [
			'label' => esc_html__('Popup', 'edumall-child'),
		]);

		$repeater->add_control('overlay_display', [
			'label'   => esc_html__('Display', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__('Hover', 'edumall-child'),
				'always' => esc_html__('Always', 'edumall-child'),
			],
			'default' => '',
		]);
		...
		$repeater->add_control('image', [
			...
			'description' => esc_html__('Choose an image that displays on left info box', 'edumall-child'),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'edumall-control-media-auto',
		]);
		...
		$this->add_group_control(Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__('Popup Image Size', 'edumall-child'),
	...
	private function add_map_markers_style_section()
	{
		$this->start_controls_section('markers_popup_style_section', [
			'label' => esc_html__('Markers Popup', 'edumall-child'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);

		$this->add_control('markers_popup_wrapper_heading', [
			'label' => esc_html__('Wrapper', 'edumall-child'),
			'type'  => Controls_Manager::HEADING,
		]);
		...
		$this->add_responsive_control('markers_popup_padding', [
			...
			'size_units' => ['px', '%', 'em'],
			'selectors'  => [
				'{{WRAPPER}} .edumall-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .edumall-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		]);
...
<?php
	...
	private function get_overlay_value($settings, $marker, $position)
	{
		...
		$this->add_render_attribute($item_key, 'class', [
			'edumall-map-overlay',
			"edumall-map-overlay-{$marker['overlay_display']}",
		...
		if ($marker['title'] !== '' || $marker['content'] !== '') {
			$info_template .= '<div class="edumall-map-overlay-template"><div class="edumall-map-overlay-content"><div class="edumall-map-overlay-info">';

			if (isset($marker['image'])) {
				$size  = \Edumall_Image::elementor_parse_image_size($settings, '80x9999', 'popup_image_size');
				$image = \Edumall_Image::get_attachment_by_id(array(
					'id'   => $marker['image']['id'],
					'size' => $size,
				));
	...
	{
		return [
			'no_label_bright_colors'  => esc_html__('No Label Bright Colors', 'edumall-child'),
			'grayscale'               => esc_html__('Grayscale', 'edumall-child'),
			'subtle_grayscale'        => esc_html__('Subtle Grayscale', 'edumall-child'),
			'apple_paps_esque'        => esc_html__('Apple Maps-esque', 'edumall-child'),
			'pale_dawn'               => esc_html__('Pale Dawn', 'edumall-child'),
			'midnight_commander'      => esc_html__('Midnight Commander', 'edumall-child'),
			'blue_water'              => esc_html__('Blue Water', 'edumall-child'),
			'retro'                   => esc_html__('Retro', 'edumall-child'),
			'paper'                   => esc_html__('Paper', 'edumall-child'),
			'ultra_light_with_labels' => esc_html__('Ultra Light with Labels', 'edumall-child'),
			'shades_of_grey'          => esc_html__('Shades Of Grey', 'edumall-child'),
			'easy_light'              => esc_html__('Easy Light', 'edumall-child'),
			'wy'                      => esc_html__('WY', 'edumall-child'),
		];
	}
	...
	{
		$icon_url = EDUMALL_ELEMENTOR_ASSETS . '/images/map-marker.png';

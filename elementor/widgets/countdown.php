...
class Widget_Countdown extends Base
{
	...
	{
		return esc_html__('Countdown', 'edumall-child');
	}
	...
	private function add_countdown_section()
	{
		$this->start_controls_section('countdown_section', [
			'label' => esc_html__('Countdown', 'edumall-child'),
		]);
		...
		$this->add_control('countdown_type', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'due_date' => esc_html__('Due Date', 'edumall-child'),
				'daily'    => esc_html__('Daily', 'edumall-child'),
			],
			'default' => 'due_date',
		]);

		$this->add_control('due_date', [
			'label'       => esc_html__('Due Date', 'edumall-child'),
			'type'        => Controls_Manager::DATE_TIME,
			'default'     => gmdate('Y-m-d H:i', strtotime('+1 month') + (get_option('gmt_offset') * HOUR_IN_SECONDS)),
			/* translators: %s: Time zone. */
			'description' => sprintf(__('Date set according to your timezone: %s.', 'edumall-child'), Utils::get_timezone_string()),
			'condition'   => [
	...
	protected function render()
	{
		...
		$days_text    = esc_html__('Days', 'edumall-child');
		$hours_text   = esc_html__('Hours', 'edumall-child');
		$minutes_text = esc_html__('Minutes', 'edumall-child');
		$seconds_text = esc_html__('Seconds', 'edumall-child');

		if (\Edumall_Helper::is_demo_site() || \Edumall_Helper::is_live_site()) {

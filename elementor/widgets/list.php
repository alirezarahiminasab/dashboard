...
class Widget_List extends Base
{
	...
	{
		return esc_html__('Modern Icon List', 'edumall-child');
	}
	...
	private function add_list_section()
	{
		$this->start_controls_section('list_section', [
			'label' => esc_html__('Icon List', 'edumall-child'),
		]);
		...
		$this->add_control('style', [
			...
			'options'      => [
				...
				'icon-border' => esc_html__('Icon Border', 'edumall-child'),
				'icon-circle' => esc_html__('Icon Circle', 'edumall-child'),
			],
		...
		$this->add_control('layout', [
			...
			'options'      => [
				'block'   => [
					'title' => esc_html__('Default', 'edumall-child'),
					'icon'  => 'eicon-editor-list-ul',
				],
				'inline'  => [
					'title' => esc_html__('Inline', 'edumall-child'),
					'icon'  => 'eicon-ellipsis-h',
				],
				'columns' => [
					'title' => esc_html__('Columns', 'edumall-child'),
					'icon'  => 'eicon-columns',
				],
			],
		...
		$this->add_control('icon', [
			'label'       => esc_html__('Default Icon', 'edumall-child'),
			'description' => esc_html__('Choose default icon for all items.', 'edumall-child'),
			'type'        => Controls_Manager::ICONS,
		]);

		$this->add_control('icon_vertical_alignment', [
			'label'                => esc_html__('Icon Alignment', 'edumall-child'),
			'type'                 => Controls_Manager::CHOOSE,
		...
		$repeater->add_control('text', [
			...
			'default'     => esc_html__('Text', 'edumall-child'),
			'label_block' => true,
		]);
		...
		$repeater->add_control('item_badge_style_hr', [
			'label'     => esc_html__('Badge Skin', 'edumall-child'),
		...
		$repeater->start_controls_tabs('badge_style_tabs', [
			'condition' => [
				'badge!' => '',
			],
		]);

		$repeater->start_controls_tab('item_badge_style_normal_tab', [
		...
		$repeater->start_controls_tab('badge_style_hover_tab', [
		...
		$this->add_control('items', [
			...
			'default'     => [
				[
					'text' => esc_html__('List Item #1', 'edumall-child'),
				],
				[
					'text' => esc_html__('List Item #2', 'edumall-child'),
				],
				[
					'text' => esc_html__('List Item #3', 'edumall-child'),
				],
			],
	...
	private function add_styling_section()
	{
		$this->start_controls_section('styling_section', [
			'label' => esc_html__('Styling', 'edumall-child'),
	...
	private function add_icon_style_section()
	{
		...
		$this->add_responsive_control('min_width', [
			'label'          => esc_html__('Min Width', 'edumall-child'),
			'type'           => Controls_Manager::SLIDER,
			'default'        => [
				'unit' => 'px',
			],
		...
		$this->add_control('icon_marker_color', [
			'label'     => esc_html__('Icon Marker', 'edumall-child'),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
...
?>
	...
	protected function content_template()
	{
		// @formatter:off
	?>
		<# var global_icon_html='' ; if ( '' !==settings.icon.value ) { var globalIconHTML=elementor.helpers.renderIcon( view, settings.icon, { 'aria-hidden' : true }, 'i' , 'object' ); global_icon_html +='<div class="edumall-icon icon"><div class="icon-view"></div>' + globalIconHTML.value + '</div>' ; } #>
			<div class="edumall-list">
				<# _.each( settings.items, function( item, index ) { #>
					<# var item_key='item' ; view.addRenderAttribute( item_key, 'class' , 'item' ); view.addRenderAttribute( item_key, 'class' , 'elementor-repeater-item-' + item._id ); var item_link_key='item_link_' + item._id; var link_tag='div' ; view.addRenderAttribute( item_link_key, 'class' , 'link' ); if ( '' !==item.link.url ) { link_tag='a' ; view.addRenderAttribute( item_link_key, 'href' , '#' ); } #>

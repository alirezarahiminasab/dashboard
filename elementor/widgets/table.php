...
class Widget_Table extends Base
{
	...
	{
		return esc_html__('Table', 'edumall-child');
	}
	...
	protected function register_controls()
	{
		...
		$this->start_controls_section('table_head_section', [
			'label' => esc_html__('Table Header', 'edumall-child'),
		]);
		...
		$table_header->add_control('action', [
			...
			'description' => esc_html__('You have started a new row. Please add new cells in your row by clicking Add Item button below.', 'edumall-child'),
			'type'        => Controls_Manager::SELECT,
			'default'     => 'Cell',
			'options'     => [
				'Row'  => esc_html__('Start New Row', 'edumall-child'),
				'Cell' => esc_html__('Add New Cell', 'edumall-child'),
			],
		]);
		...
		$table_header->add_control('text', [
			...
			'default'   => esc_html__('Sample', 'edumall-child'),
			'condition' => [
				'action' => 'Cell',
			],
		]);
		...
		$this->start_controls_section('table_body_section', [
			'label' => esc_html__('Table Content', 'edumall-child'),
		]);
...
?>
	...
	protected function content_template()
	{
		// @formatter:off
	?>
		<# view.addRenderAttribute( 'wrapper' , 'class' , 'tm-table' ); view.addRenderAttribute( 'wrapper' , 'class' , 'style-' + settings.style ); #>
			<div <# {{{ view.getRenderAttributeString( 'wrapper' ) }}} #> >
				<table>
					<# if ( settings.table_head ) { #>
						<thead>
							<# var th_count=settings.table_head.length; var thl_count=0; _.each( settings.table_head, function( item, index ) { thl_count +=1; #>
								<# if ( item.action==='Row' ) { #>
									<tr>
										<# } else { #>
											<th>{{{ item.text }}}</th>
						...
						<# } #>
							...
							<# if ( settings.table_body ) { #>
								<tbody>
									<# var tb_count=settings.table_body.length; var tbl_count=0; _.each( settings.table_body, function( item, index ) { tbl_count +=1; #>
										<# if ( item.action==='Row' ) { #>
											<tr>
												<# } else { #>
													<td>{{{ item.text }}}</td>

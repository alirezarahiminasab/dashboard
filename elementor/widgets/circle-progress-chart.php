...
class Widget_Circle_Progress_Chart extends Base
{
	...
	{
		return esc_html__('Circle Progress Chart', 'edumall-child');
	}
	...
	private function add_chart_section()
	{
		$this->start_controls_section('chart_section', [
			'label' => esc_html__('Chart', 'edumall-child'),
		]);
		...
		$this->add_control('size', [
			'label'       => esc_html__('Circle Size', 'edumall-child'),
			'description' => esc_html__('Controls the size of the pie chart circle.', 'edumall-child'),
			'type'        => Controls_Manager::NUMBER,
			'default'     => 180,
			'min'         => 100,
			'max'         => 1000,
		...
		$this->add_control('unit', [
			'label'       => esc_html__('Measuring unit', 'edumall-child'),
			'description' => esc_html__('Controls the unit of chart.', 'edumall-child'),
			'type'        => Controls_Manager::TEXT,
			'default'     => '%',
		]);

		$this->add_control('reverse', [
			'label' => esc_html__('Reverse animation and arc draw', 'edumall-child'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('line_cap', [
			'label'   => esc_html__('Line Cap', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'butt'   => esc_html__('Butt', 'edumall-child'),
				'round'  => esc_html__('Round', 'edumall-child'),
		...
		$this->add_control('line_width', [
			'label'   => esc_html__('Line Width', 'edumall-child'),
			'type'    => Controls_Manager::NUMBER,
			'default' => 3,
			'min'     => 1,
			'max'     => 50,
		...
		$this->add_control('inner_content_type', [
			'label'   => esc_html__('Inner Content', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''     => esc_html__('Animate Number', 'edumall-child'),
				'text' => esc_html__('Custom Text', 'edumall-child'),
			],
			'default' => '',
		]);
	...
	private function add_chart_style_section()
	{
		...
		$this->add_control('bar_color', [
			'label'  => esc_html__('Bar Color', 'edumall-child'),
		...
		$this->add_control('track_color', [
			'label'   => esc_html__('Track Color', 'edumall-child'),
			'type'    => Controls_Manager::COLOR,
			'default' => '#ededed',
		]);

		$this->add_control('number_color', [
			'label'     => esc_html__('Number Color', 'edumall-child'),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [

...
class Widget_Flip_Box extends Base
{
	...
	{
		return esc_html__('Advanced Flip Box', 'edumall-child');
	}
	...
	{
		return ['edumall-widget-flip-box'];
	}
	...
	private function add_front_side_content_section()
	{
		$this->start_controls_section('content_front_side_section', [
			'label' => esc_html__('Front', 'edumall-child'),
		]);

		$this->start_controls_tabs('side_a_content_tabs');

		$this->start_controls_tab('side_a_content_tab', ['label' => esc_html__('Content', 'edumall-child')]);

		$this->add_control('graphic_element_a', [
			'label'       => esc_html__('Graphic Element', 'edumall-child'),
			'type'        => Controls_Manager::CHOOSE,
			'label_block' => false,
			'options'     => [
				'none'  => [
					'title' => esc_html__('None', 'edumall-child'),
					'icon'  => 'eicon-ban',
				],
				'image' => [
					'title' => esc_html__('Image', 'edumall-child'),
					'icon'  => 'fa fa-picture-o',
				],
				'icon'  => [
					'title' => esc_html__('Icon', 'edumall-child'),
					'icon'  => 'eicon-star',
				],
			],
			'default'     => 'icon',
		...
		$this->add_control('image_a', [
			'label'     => esc_html__('Choose Image', 'edumall-child'),
		...
		$this->add_control('icon_a', [
			'label'     => esc_html__('Icon', 'edumall-child'),
		...
		$this->add_control('title_text_a', [
			'label'       => esc_html__('Title & Description', 'edumall-child'),
			'type'        => Controls_Manager::TEXT,
			'default'     => esc_html__('This is the heading', 'edumall-child'),
			'placeholder' => esc_html__('Enter your title', 'edumall-child'),
			'dynamic'     => [
				'active' => true,
			],
			'label_block' => true,
		...
		$this->add_control('description_text_a', [
			'label'       => esc_html__('Description', 'edumall-child'),
			'type'        => Controls_Manager::TEXTAREA,
			'default'     => 'Lorem ipsum dolor sit amet consectetur adipiscing elit dolor',
			'placeholder' => esc_html__('Enter your description', 'edumall-child'),
			'separator'   => 'none',
			'dynamic'     => [
				'active' => true,
			],
		...
		$this->start_controls_tab('side_a_background_tab', ['label' => esc_html__('Background', 'edumall-child')]);
		...
		$this->add_control('background_overlay_a', [
			'label'     => esc_html__('Background Overlay', 'edumall-child'),
	...
	private function add_back_side_content_section()
	{
		$this->start_controls_section('content_back_side_section', [
			'label' => esc_html__('Back', 'edumall-child'),
		]);

		$this->start_controls_tabs('side_b_content_tabs');

		$this->start_controls_tab('side_b_content_tab', ['label' => esc_html__('Content', 'edumall-child')]);
		...
		$this->add_control('link', [
			'label'       => esc_html__('Link', 'edumall-child'),
			'type'        => Controls_Manager::URL,
			'dynamic'     => [
				'active' => true,
			],
			'placeholder' => esc_html__('https://your-link.com', 'edumall-child'),
			'separator'   => 'before',
		]);

		$this->add_control('link_click', [
			'label'     => esc_html__('Apply Link On', 'edumall-child'),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				'box'    => esc_html__('Whole Box', 'edumall-child'),
				'button' => esc_html__('Button Only', 'edumall-child'),
			],
			'default'   => 'button',
			'condition' => [
				'link[url]!' => '',
		...
		$this->start_controls_tab('side_b_background_tab', ['label' => esc_html__('Background', 'edumall-child')]);
	...
	private function add_box_settings_section()
	{
		$this->start_controls_section('section_box_settings', [
			'label' => esc_html__('Settings', 'edumall-child'),
		]);

		$this->add_responsive_control('min_height', [
			'label'      => esc_html__('Min Height', 'edumall-child'),
			...
			'size_units' => ['px'],
			'selectors'  => [
				'{{WRAPPER}} .edumall-flip-box' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		]);

		$this->add_control('border_radius', [
			'label'      => esc_html__('Border Radius', 'edumall-child'),
			'type'       => Controls_Manager::SLIDER,
			'size_units' => ['px', '%'],
			'range'      => [
				'px' => [
		...
		$this->add_control('flip_effect', [
			'label'        => esc_html__('Flip Effect', 'edumall-child'),
			'type'         => Controls_Manager::SELECT,
			'default'      => 'flip',
			'options'      => [
				'flip'     => 'Flip',
			...
			'prefix_class' => 'edumall-flip-box--effect-',
		]);

		$this->add_control('flip_direction', [
			'label'        => esc_html__('Flip Direction', 'edumall-child'),
			'type'         => Controls_Manager::SELECT,
			'default'      => 'up',
			'options'      => [
				'left'  => esc_html__('Left', 'edumall-child'),
				'right' => esc_html__('Right', 'edumall-child'),
				'up'    => esc_html__('Up', 'edumall-child'),
				'down'  => esc_html__('Down', 'edumall-child'),
			],
			'condition'    => [
				'flip_effect!' => [
					'fade',
			...
			'prefix_class' => 'edumall-flip-box--direction-',
		]);

		$this->add_control('flip_3d', [
			'label'        => esc_html__('3D Depth', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'label_on'     => esc_html__('On', 'edumall-child'),
			'label_off'    => esc_html__('Off', 'edumall-child'),
			'return_value' => 'edumall-flip-box--3d',
			'default'      => '',
			'prefix_class' => '',
			'condition'    => [
				'flip_effect' => 'flip',
	...
	private function add_front_side_box_style_section()
	{
		$this->start_controls_section('box_style_front_side_section', [
			'label' => esc_html__('Front - Box', 'edumall-child'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);

		$this->add_responsive_control('padding_a', [
			'label'      => esc_html__('Padding', 'edumall-child'),
		...
		$this->add_control('alignment_a', [
			'label'       => esc_html__('Alignment', 'edumall-child'),
			'type'        => Controls_Manager::CHOOSE,
			'label_block' => false,
			'options'     => [
				'left'   => [
					'title' => esc_html__('Left', 'edumall-child'),
					'icon'  => 'eicon-text-align-left',
				],
				'center' => [
					'title' => esc_html__('Center', 'edumall-child'),
					'icon'  => 'eicon-text-align-center',
				],
				'right'  => [
					'title' => esc_html__('Right', 'edumall-child'),
		...
		$this->add_control('vertical_position_a', [
			'label'                => esc_html__('Vertical Position', 'edumall-child'),
			'type'                 => Controls_Manager::CHOOSE,
			'label_block'          => false,
			'options'              => [
				'top'    => [
					'title' => esc_html__('Top', 'edumall-child'),
					'icon'  => 'eicon-v-align-top',
				],
				'middle' => [
					'title' => esc_html__('Middle', 'edumall-child'),
					'icon'  => 'eicon-v-align-middle',
				],
				'bottom' => [
					'title' => esc_html__('Bottom', 'edumall-child'),
	...
	private function add_front_side_image_style_section()
	{
		$this->start_controls_section('image_style_front_side_section', [
			'label'     => esc_html__('Front - Image', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'graphic_element_a' => 'image',
			],
		...
		$this->add_control('image_spacing_a', [
			'label'     => esc_html__('Spacing', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
		...
		$this->add_control('image_width_a', [
			'label'      => esc_html__('Size', 'edumall-child') . ' (%)',
			'type'       => Controls_Manager::SLIDER,
			'size_units' => ['%'],
			'default'    => [
				'unit' => '%',
		...
		$this->add_control('image_opacity_a', [
			'label'     => esc_html__('Opacity', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'max'  => 1,
	...
	private function add_front_side_icon_style_section()
	{
		$this->start_controls_section('icon_style_front_side_section', [
			'label'     => esc_html__('Front - Icon', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'graphic_element_a' => 'icon',
			],
		...
		$this->add_control('icon_spacing_a', [
			...
			'selectors' => [
				'{{WRAPPER}} .front-side .edumall-icon-wrap' => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('icon_size_a', [
			...
			'selectors' => [
				'{{WRAPPER}} .front-side .edumall-icon-view, {{WRAPPER}} .front-side .edumall-icon' => 'font-size: {{SIZE}}{{UNIT}};',
			],
		]);

		$this->add_control('icon_rotate_a', [
			'label'     => esc_html__('Rotate', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'default'   => [
				'unit' => 'deg',
			],
			'selectors' => [
				'{{WRAPPER}} .front-side .edumall-icon > svg, {{WRAPPER}} .front-side .edumall-icon > i' => 'transform: rotate({{SIZE}}{{UNIT}});',
			],
		]);
	...
	private function add_front_side_heading_style_section()
	{
		$this->start_controls_section('heading_style_front_side_section', [
			'label'     => esc_html__('Front - Heading', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'title_text_a!' => '',
			],
	...
	private function add_front_side_description_style_section()
	{
		$this->start_controls_section('description_style_front_side_section', [
			'label'     => esc_html__('Front - Description', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'description_text_a!' => '',
			],
	...
	private function add_back_side_box_style_section()
	{
		$this->start_controls_section('box_style_back_side_section', [
			'label' => esc_html__('Back - Box', 'edumall-child'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);
	...
	private function add_back_side_image_style_section()
	{
		$this->start_controls_section('image_style_back_side_section', [
			'label'     => esc_html__('Back - Image', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'graphic_element_b' => 'image',
			],
	...
	private function add_back_side_icon_style_section()
	{
		$this->start_controls_section('icon_style_back_side_section', [
			'label'     => esc_html__('Back - Icon', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'graphic_element_b' => 'icon',
			],
		...
		$this->add_control('icon_spacing_b', [
			...
			'selectors' => [
				'{{WRAPPER}} .back-side .edumall-icon-wrap' => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('icon_size_b', [
			...
			'selectors' => [
				'{{WRAPPER}} .back-side .edumall-icon-view, {{WRAPPER}} .back-side .edumall-icon' => 'font-size: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_control('icon_rotate_b', [
			...
			'selectors' => [
				'{{WRAPPER}} .back-side .edumall-icon > svg, {{WRAPPER}} .back-side .edumall-icon > i' => 'transform: rotate({{SIZE}}{{UNIT}});',
			],
		]);
	...
	private function add_back_side_heading_style_section()
	{
		$this->start_controls_section('heading_style_back_side_section', [
			'label'     => esc_html__('Back - Heading', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'title_text_b!' => '',
			],
	...
	private function add_back_side_description_style_section()
	{
		$this->start_controls_section('description_style_back_side_section', [
			'label'     => esc_html__('Back - Description', 'edumall-child'),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'description_text_b!' => '',
			],
...
?>
	...
	private function print_graphic_icon(array $settings, $side = 'a')
	{
		...
		switch ($settings["icon_{$side}_color_type"]) {
			case 'gradient':
	...
	?>
		<div class="heading-wrap">
			<h3 class="heading">
				<?php echo wp_kses($settings["title_text_{$side}"], 'edumall-default'); ?>
			</h3>
		</div>
	<?php
	}
	...
	?>
		<div class="description-wrap">
			<div class="description">
				<?php echo wp_kses($settings["description_text_{$side}"], 'edumall-default'); ?>
			</div>
		</div>
	<?php
	}
	...
	?>
		<# var svg_id='<?php echo esc_html($id); ?>' ; #>
			...
			<# var icon_a_svg_id='icon-a-' + svg_id; var icon_b_svg_id='icon-b-' + svg_id; // Image Front Side. if ( 'image'===settings.graphic_element_a && '' !==settings.image_a.url ) { var imageA={ id: settings.image_a.id, url: settings.image_a.url, size: settings.image_a_size, dimension: settings.image_a_custom_dimension, model: view.getEditModel() }; var imageSideAUrl=elementor.imagesManager.getImageUrl( imageA ); view.addRenderAttribute( 'image-a' , 'src' , imageSideAUrl ); } // Image Back Side. if ( 'image'===settings.graphic_element_b && '' !==settings.image_b.url ) { var imageB={ id: settings.image_b.id, url: settings.image_b.url, size: settings.image_b_size, dimension: settings.image_b_custom_dimension, model: view.getEditModel() }; var imageSideBUrl=elementor.imagesManager.getImageUrl( imageB ); view.addRenderAttribute( 'image-b' , 'src' , imageSideBUrl ); } var wrapperTag='div' ; if ( '' !==settings.link.url && 'box'===settings.link_click ) { wrapperTag='a' ; } #>
				<div class="edumall-flip-box">
							...
							<div class="layer-content">
								<# if ( 'image'===settings.graphic_element_a && '' !==settings.image_a.url ) { #>
									...
									<# } else if ( 'icon'===settings.graphic_element_a ) { #>
										<# view.addRenderAttribute( 'iconA' , 'class' , 'edumall-icon icon' ); if ( 'svg'===settings.icon_a.library ) { view.addRenderAttribute( 'iconA' , 'class' , 'edumall-svg-icon' ); } switch( settings.icon_a_color_type ) { case 'gradient' : view.addRenderAttribute( 'iconA' , 'class' , 'edumall-gradient-icon' ); break; case 'classic' : view.addRenderAttribute( 'iconA' , 'class' , 'edumall-solid-icon' ); break; } var iconAHTML=elementor.helpers.renderIcon( view, settings.icon_a, { 'aria-hidden' : true }, 'i' , 'object' ); #>
											<div class="edumall-icon-wrap">
												<div class="edumall-icon-view">
													<div {{{ view.getRenderAttributeString( 'iconA' ) }}}>
														<# if ( iconAHTML.rendered ) { #>
															<# var stop_a=settings.icon_a_color_a_stop.size + settings.icon_a_color_a_stop.unit; var stop_b=settings.icon_a_color_b_stop.size + settings.icon_a_color_b_stop.unit; var iconValue=iconAHTML.value; if ( typeof iconValue==='string' ) { var strokeAttr='stroke="' + 'url(#' + icon_a_svg_id + ')"' ; var fillAttr='fill="' + 'url(#' + icon_a_svg_id + ')"' ; iconValue=iconValue.replace(new RegExp(/stroke="#(.*?)" /, 'g' ), strokeAttr); iconValue=iconValue.replace(new RegExp(/fill="#(.*?)" /, 'g' ), fillAttr); } #>
							...
							<div class="layer-content">
								<# if ( 'image'===settings.graphic_element_b && '' !==settings.image_b.url ) { #>
									...
									<# } else if ( 'icon'===settings.graphic_element_b ) { #>
										<# view.addRenderAttribute( 'iconB' , 'class' , 'edumall-icon icon' ); if ( 'svg'===settings.icon_b.library ) { view.addRenderAttribute( 'iconB' , 'class' , 'edumall-svg-icon' ); } switch( settings.icon_b_color_type ) { case 'gradient' : view.addRenderAttribute( 'iconB' , 'class' , 'edumall-gradient-icon' ); break; case 'classic' : view.addRenderAttribute( 'iconB' , 'class' , 'edumall-solid-icon' ); break; } var iconBHTML=elementor.helpers.renderIcon( view, settings.icon_b, { 'aria-hidden' : true }, 'i' , 'object' ); #>
											<div class="edumall-icon-wrap">
												<div class="edumall-icon-view">
													<div {{{ view.getRenderAttributeString( 'iconB' ) }}}>
														<# if ( iconBHTML.rendered ) { #>
															<# var stop_a=settings.icon_b_color_a_stop.size + settings.icon_b_color_a_stop.unit; var stop_b=settings.icon_b_color_b_stop.size + settings.icon_b_color_b_stop.unit; var iconValue=iconBHTML.value; if ( typeof iconValue==='string' ) { var strokeAttr='stroke="' + 'url(#' + icon_b_svg_id + ')"' ; var fillAttr='fill="' + 'url(#' + icon_b_svg_id + ')"' ; iconValue=iconValue.replace(new RegExp(/stroke="#(.*?)" /, 'g' ), strokeAttr); iconValue=iconValue.replace(new RegExp(/fill="#(.*?)" /, 'g' ), fillAttr); } #>
														...
														<# if ( settings.description_text_b ) { #>
															...
															<# } #>
																...
																<# if ( settings.button_text || settings.button_icon.value ) { #>
																	<# var buttonIconHTML=elementor.helpers.renderIcon( view, settings.button_icon, { 'aria-hidden' : true }, 'i' , 'object' ); var buttonTag='div' ; view.addRenderAttribute( 'button' , 'class' , 'tm-button style-' + settings.button_style ); view.addRenderAttribute( 'button' , 'class' , 'tm-button-' + settings.button_size ); if ( '' !==settings.link.url && 'button'===settings.link_click ) { buttonTag='a' ; view.addRenderAttribute( 'button' , 'href' , '#' ); } if ( settings.button_icon.value ) { view.addRenderAttribute( 'button' , 'class' , 'icon-' + settings.button_icon_align ); } view.addRenderAttribute( 'button-icon' , 'class' , 'button-icon' ); #>

...
class Widget_Button extends Base
{
	...
	{
		return esc_html__('Advanced Button', 'edumall-child');
	}
	...
	private function add_button_settings_section()
	{
		$this->start_controls_section('button_settings_section', [
			'label' => esc_html__('Button', 'edumall-child'),
		]);
		...
		$this->add_control('text', [
			...
			'default'     => esc_html__('Click here', 'edumall-child'),
			'placeholder' => esc_html__('Click here', 'edumall-child'),
		]);
		...
		$this->add_control('action', [
			'label'       => esc_html__('Action', 'edumall-child'),
			'description' => esc_html__('To make smooth scroll action work then input button link like this: #about-us-section.', 'edumall-child'),
			'type'        => Controls_Manager::SELECT,
			'default'     => '',
			'options'     => [
				''              => esc_html__('Normal', 'edumall-child'),
				'smooth_scroll' => esc_html__('Smooth scroll to a section', 'edumall-child'),
				'popup_video'   => esc_html__('Open Popup Video', 'edumall-child'),
			],
		]);
		...
		$this->add_control('link', [
			...
			'placeholder' => esc_attr__('https://your-link.com', 'edumall-child'),
			'default'     => [
				'url' => '#',
			],
		]);
		...
		$this->add_control('button_css_id', [
			...
			'default'     => '',
			'title'       => esc_html__('Add your custom id WITHOUT the Pound key. e.g: my-id', 'edumall-child'),
			'label_block' => false,
			'description' => wp_kses(__('Please make sure the ID is unique and not used elsewhere on the page this form is displayed. This field allows <code>A-z 0-9</code> & underscore chars without spaces.', 'edumall-child'), 'edumall-default'),
			'separator'   => 'before',
		]);

		$this->add_control('button_css_class', [
			'label'       => esc_html__('Button Class', 'edumall-child'),
			'type'        => Controls_Manager::TEXT,
			'dynamic'     => [
				'active' => true,
			],
			'default'     => '',
			'title'       => esc_html__('Add your custom classes WITHOUT the dot key. e.g: my-class', 'edumall-child'),
			'label_block' => false,
		]);
	...
	private function add_button_badge_section()
	{
		$this->start_controls_section('button_badge_section', [
			'label' => esc_html__('Badge', 'edumall-child'),
		]);
		...
		$this->add_control('badge_text', [
			...
			'placeholder' => esc_html__('$59', 'edumall-child'),
		]);
	...
	private function add_wrapper_section()
	{
		...
		$this->add_control('size', [
			...
			'options'        => [
				...
				'xs'     => esc_html__('Extra Small', 'edumall-child'),
				'sm'     => esc_html__('Small', 'edumall-child'),
				'nm'     => esc_html__('Normal', 'edumall-child'),
				'lg'     => esc_html__('Large', 'edumall-child'),
				'xl'     => esc_html__('Extra Large', 'edumall-child'),
			],
			'style_transfer' => true,
		]);
		...
		$this->add_control('rounded', [
			'label'      => esc_html__('Rounded', 'edumall-child'),
			'type'       => Controls_Manager::DIMENSIONS,
			'size_units' => ['px', '%'],
			'selectors'  => [
	...
	private function add_skin_section()
	{
		$this->start_controls_section('skin_section', [
			'label' => esc_html__('Skin', 'edumall-child'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);
		...
		$this->add_control('button_border_color', [
			'label'     => esc_html__('Border', 'edumall-child'),
		...
		$this->add_control('hover_animation', [
			'label' => esc_html__('Hover Animation', 'edumall-child'),
	...
	private function add_text_style_section()
	{
		...
		$this->add_control('button_line_color', [
			'label'     => esc_html__('Line', 'edumall-child'),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
	...
	private function add_icon_style_section()
	{
		...
		$this->add_responsive_control('icon_font_size', [
			'label'     => esc_html__('Font Size', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 8,
...
?>
	...
	protected function content_template()
	{
		// @formatter:off
	?>
		<# view.addRenderAttribute( 'text' , 'class' , 'button-text' ); view.addInlineEditingAttributes( 'text' , 'none' ); var iconHTML=elementor.helpers.renderIcon( view, settings.icon, { 'aria-hidden' : true }, 'i' , 'object' ); #>
			<div class="tm-button-wrapper">
				<a id="{{ settings.button_css_id }}" class="tm-button style-{{ settings.style }} tm-button-{{ settings.size }} elementor-animation-{{ settings.hover_animation }} icon-{{ settings.icon_align }} {{ settings.button_css_class }}" href="{{ settings.link.url }}" role="button">

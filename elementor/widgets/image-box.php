...
class Widget_Image_Box extends Base
{
	...
	{
		return esc_html__('Image Box', 'edumall-child');
	}
	...
	private function add_image_box_section()
	{
		$this->start_controls_section('image_section', [
			'label' => esc_html__('Image Box', 'edumall-child'),
		]);

		$this->add_control('style', [
			'label'   => esc_html__('Style', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''   => esc_html__('None', 'edumall-child'),
				'01' => '01',
				'02' => '02',
				'03' => '03',
			],
		...
		$this->add_control('hover_effect', [
			'label'        => esc_html__('Hover Effect', 'edumall-child'),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''         => esc_html__('None', 'edumall-child'),
				'zoom-in'  => esc_html__('Zoom In', 'edumall-child'),
				'zoom-out' => esc_html__('Zoom Out', 'edumall-child'),
				'move-up'  => esc_html__('Move Up', 'edumall-child'),
			],
			'default'      => '',
		...
		$this->add_control('image_heading', [
			'label'     => esc_html__('Image', 'edumall-child'),
		...
		$this->add_control('image_position', [
			'label'     => esc_html__('Image Position', 'edumall-child'),
		...
		$this->add_control('content_vertical_alignment', [
			'label'     => esc_html__('Vertical Alignment', 'edumall-child'),
			'type'      => Controls_Manager::CHOOSE,
		...
		$this->add_control('title_text', [
			'label'       => esc_html__('Text', 'edumall-child'),
		...
		$this->add_control('title_size', [
			'label'   => esc_html__('HTML Tag', 'edumall-child'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'h1'   => 'H1',
				'h2'   => 'H2',
		...
		$this->add_control('view', [
			'label'   => esc_html__('View', 'edumall-child'),
	...
	private function add_box_style_section()
	{
		$this->start_controls_section('box_style_section', [
			'label' => esc_html__('Box', 'edumall-child'),
		...
		$this->add_responsive_control('box_max_width', [
			'label'      => esc_html__('Max Width', 'edumall-child'),
		...
		$this->add_responsive_control('box_horizontal_alignment', [
			'label'                => esc_html__('Horizontal Alignment', 'edumall-child'),
			'label_block'          => true,
			'type'                 => Controls_Manager::CHOOSE,
	...
	private function add_image_style_section()
	{
		...
		$this->add_responsive_control('image_wrap_height', [
			'label'     => esc_html__('Wrap Height', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
			...
			'selectors' => [
				'{{WRAPPER}} .edumall-image' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('image_vertical_align', [
			...
			'selectors'            => [
				'{{WRAPPER}} .edumall-image' => 'align-items: {{VALUE}}',
			],
		]);

		$this->add_responsive_control('image_space_top', [
			'label'     => esc_html__('Offset Top', 'edumall-child'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
		...
		$this->add_responsive_control('image_space', [
			...
			'selectors' => [
				'{{WRAPPER}} .image-position-right .image-wrap' => 'margin-left: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}} .image-position-left .image-wrap'  => 'margin-right: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}} .image-position-top .image-wrap'   => 'margin-bottom: {{SIZE}}{{UNIT}};',
				//'(mobile){{WRAPPER}} .image-wrap'               => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		]);

		$this->add_responsive_control('image_width', [
			'label'          => esc_html__('Width', 'edumall-child'),
			'type'           => Controls_Manager::SLIDER,
			'default'        => [
				'unit' => '%',
			],
	...
	private function add_content_style_section()
	{
		$this->start_controls_section('content_style_section', [
			'label' => esc_html__('Content', 'edumall-child'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);
		...
		$this->add_responsive_control('caption_padding', [
			...
			'size_units' => ['px', '%', 'em'],
			'selectors'  => [
				'{{WRAPPER}} .edumall-box .box-caption-wrap' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		]);
		...
		$this->add_control('title_hover_color', [
			...
			'default'   => '',
			'selectors' => [
				'{{WRAPPER}} .edumall-box:hover .title' => 'color: {{VALUE}};',
			],
		]);
		...
		$this->add_control('description_hover_color', [
			...
			'default'   => '',
			'selectors' => [
				'{{WRAPPER}} .edumall-box:hover .description' => 'color: {{VALUE}};',
			],
		]);
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', 'tm-image-box edumall-box');
...
?>
	...
	protected function content_template()
	{
		// @formatter:off
	?>
		<# view.addRenderAttribute( 'wrapper' , 'class' , 'tm-image-box edumall-box' ); view.addRenderAttribute( 'wrapper' , 'class' , 'style-' + settings.style ); view.addRenderAttribute( 'wrapper' , 'class' , 'image-position-' + settings.image_position ); view.addRenderAttribute( 'wrapper' , 'class' , 'content-alignment-' + settings.content_vertical_alignment ); var boxTag='div' ; if( '' !==settings.link.url && 'box'===settings.link_click ) { boxTag='a' ; view.addRenderAttribute( 'wrapper' , 'href' , '#' ); view.addRenderAttribute( 'wrapper' , 'class' , 'link-secret' ); } var imageHTML='' ; if ( settings.image.url ) { var image={ id: settings.image.id, url: settings.image.url, size: settings.image_size, dimension: settings.image_custom_dimension, model: view.getEditModel() }; var image_url=elementor.imagesManager.getImageUrl( image ); view.addRenderAttribute( 'image' , 'src' , image_url ); imageHTML='<div class="edumall-image image"><img ' + view.getRenderAttributeString( 'image' ) + ' /></div>' ; } var title_over_image=false; if ( '03'===settings.style ) { title_over_image=true; } #>
	...
	private function print_description(array $settings)
	{
		...
		?>
			<div <?php $this->print_render_attribute_string('description_text'); ?>>
				<?php echo wp_kses($settings['description_text'], 'edumall-default'); ?>
			</div>
	<?php
	}
}

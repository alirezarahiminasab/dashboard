...
class Widget_Twitter extends Base
{
	...
	{
		return esc_html__('Twitter', 'edumall-child');
	}
	...
	protected function register_controls()
	{
		...
		$this->add_control('consumer_key', [
			'label' => esc_html__('Consumer Key', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);

		$this->add_control('consumer_secret', [
			'label' => esc_html__('Consumer Secret', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);

		$this->add_control('access_token', [
			'label' => esc_html__('Access Token', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);

		$this->add_control('access_token_secret', [
			'label' => esc_html__('Access Token Secret', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);

		$this->add_control('username', [
			'label' => esc_html__('Twitter Username', 'edumall-child'),
			'type'  => Controls_Manager::TEXT,
		]);
		...
		$this->add_control(
			...
			[
				...
				'label_on'     => esc_html__('Yes', 'edumall-child'),
				'label_off'    => esc_html__('No', 'edumall-child'),
				'return_value' => '1',
			]
		);
			...
			<?php foreach ($tweets as $tweet) : ?>
				...
				<div class="tweet">
					...
					<?php if ('1' === $settings['show_date']) : ?>
						<span class="tweet-date"><?php echo wp_date('F d, Y', strtotime($tweet['created_at'])); ?></span>
					<?php endif; ?>
				</div>
			<?php endforeach; ?>
		</div>

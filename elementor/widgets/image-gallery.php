...
class Widget_Image_Gallery extends Base
{
	...
	{
		return esc_html__('Image Gallery', 'edumall-child');
	}
	...
	private function add_content_section()
	{
		...
		$this->add_control('gallery', [
			'label'      => esc_html__('Add Images', 'edumall-child'),
	...
	private function add_layout_section()
	{
		...
		$this->add_control('lightbox_enable', [
			'label'        => esc_html__('Lightbox', 'edumall-child'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);
	...
	protected function render()
	{
		...
		$this->add_render_attribute('grid-wrapper', 'class', 'edumall-grid-wrapper');
		...
		if (!empty($settings['lightbox_enable'])) {
			$this->add_render_attribute('grid-wrapper', 'class', 'edumall-light-gallery');
		}
?>
	...
	private function print_image($image, $image_size)
	{
		...
		if (!empty($settings['lightbox_enable'])) {
			...
			$image_full = \Edumall_Image::get_attachment_info($image['id']);

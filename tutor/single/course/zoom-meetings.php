	...
	<div class="tutor-single-course-segment tutor-course-topics-wrap">
		<div class="tutor-course-topics-header">
			<div class="tutor-course-topics-header-left">
				<h4 class="tutor-segment-title"><?php esc_html_e('Zoom Live Meetings', 'edumall-child'); ?></h4>
			</div>
		</div>
						...
						<div class="tutor-zoom-meeting-detail">
							<h3>
								...
								<?php if ($zoom_meeting->is_expired) {
									echo '<span class="tutor-zoom-label">' . esc_html__('Expired', 'edumall-child') . '</span>';
								} else if ($zoom_meeting->is_started) {
									echo '<span class="tutor-zoom-label tutor-zoom-live-label">' . esc_html__('Live', 'edumall-child') . '</span>';
								}
								?>
							</h3>
							<div>
								<p><?php esc_html_e('ID', 'edumall-child'); ?>:
									<span><?php echo esc_html($meeting_data['id']); ?></span> <i class="tutor-icon-copy"></i>
								</p>
								<p><?php esc_html_e('Password', 'edumall-child'); ?>:
									<span><?php echo esc_html($meeting_data['password']); ?></span> <i class="tutor-icon-copy"></i>
								</p>
							</div>
						</div>
					...
					<div class="tutor-course-lessons tutor-zoom-meeting-session" style="display: <?php echo ($index == 0) ? 'block' : 'none'; ?>">
						<?php if ($zoom_meeting->is_expired) { ?>
							<div class="msg-expired-section">
								<img src="<?php echo esc_url(TUTOR_ZOOM()->url . 'assets/images/zoom-icon-expired.png'); ?>" alt="<?php esc_attr_e('Zoom expired', 'edumall-child'); ?>" />
								<div>
									<h3><?php esc_html_e('The video conference has expired', 'edumall-child'); ?></h3>
									<p><?php esc_html_e('Please contact your instructor for further information', 'edumall-child'); ?></p>
								</div>
							</div>
						<?php
						} else { ?>
							...
							<div class="session-link">
								<p><?php esc_html_e('Host Email', 'edumall-child'); ?>
									: <?php echo esc_html($meeting_data['host_email']); ?></p>
								<a href="<?php echo esc_url(get_permalink($meeting->ID)); ?>" class="tutor-btn bordered-btn"><?php esc_html_e('Continue to Meeting', 'edumall-child'); ?></a>
							</div>
						<?php } ?>
					</div>
				</div>

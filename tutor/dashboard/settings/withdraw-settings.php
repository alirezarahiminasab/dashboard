...
?>
<h3><?php esc_html_e('Settings', 'edumall-child') ?></h3>
...
<div class="tutor-dashboard-setting-withdraw tutor-dashboard-content-inner">
	...
	<form id="tutor-withdraw-account-set-form" action="" method="post" class="dashboard-settings-form dashboard-settings-withdraw-form">
		<div class="dashboard-content-box dashboard-content-box-small">
			<h4 class="dashboard-content-box-title"><?php esc_html_e('Select a withdraw method', 'edumall-child'); ?></h4>
			<?php if (tutor_utils()->count($tutor_withdrawal_methods)) : ?>
				...
				<div class="withdraw-method-select-wrap">
					<?php foreach ($tutor_withdrawal_methods as $method_id => $method) : ?>
						<div class="withdraw-method-select <?php echo 'withdraw-method-' . $method_id; ?>" data-withdraw-method="<?php echo esc_attr($method_id); ?>">
							...
							<label for="withdraw_method_select_<?php echo esc_attr($method_id); ?>">
								<span class="method-name"><?php echo tutor_utils()->avalue_dot('method_name', $method); ?></span>
								<span class="method-amount"><?php esc_html_e('Min withdraw', 'edumall-child'); ?><?php echo tutor_utils()->tutor_price($min_withdraw_amount);
																													?></span>
							</label>
						</div>
					<?php endforeach; ?>
				...
				<div class="dashboard-withdraw-method-forms-wrap">
					...
					<?php foreach ($tutor_withdrawal_methods as $method_id => $method) : ?>
						...
						<div data-withdraw-form="<?php echo esc_attr($method_id); ?>" class="tutor-row withdraw-method-form" style="<?php echo esc_attr($old_method_key != $method_id ? 'display: none;' : ''); ?>">
							...
							?>
									<div class="withdraw-method-field-wrap tutor-form-group <?php echo 'withdraw-method-field-' . $field_name . ' ' . $field['type']; ?> ">
										<?php
		...
		<?php if (tutor_utils()->count($tutor_withdrawal_methods)) : ?>
			<div class="withdraw-account-save-btn-wrap">
				<button type="submit" class="tutor_set_withdraw_account_btn tutor-btn" name="withdraw_btn_submit"><?php esc_html_e('Save Withdraw Account', 'edumall-child'); ?></button>
			</div>
		<?php endif; ?>
	</form>
</div>
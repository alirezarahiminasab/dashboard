...
if (!$assignment_submitted_id) : ?>
	<div class="dashboard-no-content-found">
		<?php esc_html_e('Sorry, but you are looking for something that isn\'t here.', 'edumall-child'); ?>
	</div>
	<?php return; ?>
<?php endif; ?>
...
<?php if ($submitted_assignment) : ?>
	...
	<h3>
		<?php echo sprintf(esc_html__('Assignment: %s', 'edumall-child'), $assignment_link); ?>
	</h3>
	...
	<form action="" method="post">
		<div class="dashboard-content-box">
			<div class="assignment-review-header">
				<h4>
					<?php esc_html_e('Course', 'edumall-child'); ?> :
			...
			<div class="dashboard-assignment-review">
				<h4><?php esc_html_e('Assignment Description:', 'edumall-child'); ?></h4>
				...
				if ($attached_files) :
				?>
					<h5><?php esc_html_e('Attach assignment file(s)', 'edumall-child'); ?></h5>
						...
						<?php if (tutor_utils()->count($attached_files)) : ?>
							...
							<?php foreach ($attached_files as $attached_file) : ?>
								<div class="uploaded-files">
									<a href="<?php echo esc_url($upload_baseurl . tutor_utils()->array_get('uploaded_path', $attached_file)); ?>" target="_blank"> <i class="tutor-icon-upload-file"></i> <?php echo tutor_utils()->array_get('name', $attached_file); ?>
									</a>
								</div>
							<?php endforeach; ?>
						<?php endif; ?>
			...
			<div class="tutor-assignment-evaluate-wraps">
				<h3><?php esc_html_e('Evaluation', 'edumall-child'); ?></h3>
				...
				<div class="tutor-assignment-evaluate-row">
					<div class="tutor-option-field-label">
						<label for="evaluate-assignment-mark"><?php esc_html_e('Your Mark', 'edumall-child'); ?></label>
					</div>
					<div class="tutor-option-field">
						<input type="number" name="evaluate_assignment[assignment_mark]" value="<?php echo esc_attr(sprintf('%s', $given_mark ? $given_mark : 0)); ?>" max="<?php echo esc_attr($max_mark); ?>" min="0" id="evaluate-assignment-mark">
						<p class="desc"><?php echo sprintf(esc_html__('Mark this assignment out of %s', 'edumall-child'), "<strong>{$max_mark}</strong>"); ?></p>
					</div>
				</div>
				<div class="tutor-assignment-evaluate-row">
					<div class="tutor-option-field-label">
						<label for="evaluate-assignment-instructor-note"><?php esc_html_e('Write a note', 'edumall-child'); ?></label>
					</div>
					<div class="tutor-option-field">
						<textarea name="evaluate_assignment[instructor_note]" id="evaluate-assignment-instructor-note"><?php echo esc_html($instructor_note); ?></textarea>
						<p class="desc"><?php esc_html_e('Write a note to students about this submission', 'edumall-child'); ?></p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="dashboard-form-submit-wrap">
			<button type="submit" class="tutor-button tutor-button-primary tutor-success"><?php esc_html_e('Evaluate this submission', 'edumall-child'); ?></button>
		</div>
	</form>
<?php else : ?>
	<div class="dashboard-no-content-found">
		<?php esc_html_e('Assignments submission not found or not completed.', 'edumall-child'); ?>
	</div>
<?php endif;

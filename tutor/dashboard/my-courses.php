...
<h3><?php esc_html_e('My Courses', 'edumall-child'); ?></h3>
	...
	<ul class="tutor-nav">
		<li class="tutor-nav-item">
			<a class="tutor-nav-link<?php echo $active_tab == 'my-courses' ? ' is-active' : ''; ?>" href="<?php echo esc_url(tutor_utils()->get_tutor_dashboard_page_permalink('my-courses')); ?>">
				<?php echo esc_html(sprintf('%1$s (%2$s)', __('Publish', 'edumall-child'), $count_map['publish'])); ?>
			</a>
		</li>
		<li class="tutor-nav-item">
			<a class="tutor-nav-link<?php echo $active_tab == 'my-courses/pending-courses' ? ' is-active' : ''; ?>" href="<?php echo esc_url(tutor_utils()->get_tutor_dashboard_page_permalink('my-courses/pending-courses')); ?>">
				<?php echo esc_html(sprintf('%1$s (%2$s)', __('Pending', 'edumall-child'), $count_map['pending'])); ?>
			</a>
		</li>
		<li class="tutor-nav-item">
			<a class="tutor-nav-link<?php echo $active_tab == 'my-courses/draft-courses' ? ' is-active' : ''; ?>" href="<?php echo esc_url(tutor_utils()->get_tutor_dashboard_page_permalink('my-courses/draft-courses')); ?>">
				<?php echo esc_html(sprintf('%1$s (%2$s)', __('Draft', 'edumall-child'), $count_map['draft'])); ?>
			</a>
		</li>
	</ul>
</div>
...
<div class="tutor-dashboard-content-inner edumall-animation-zoom-in">
	<?php if (is_array($results) && count($results)) : ?>
		...
		<?php foreach ($results as $post) : ?>
			...
			?>
			<div id="<?php echo $row_id ?>" class="edumall-box tutor-mycourse-wrap tutor-mycourse-<?php the_ID(); ?>">
					...
					<div class="tutor-meta tutor-course-metadata">
						...
						<ul class="course-meta">
							...
							<li class="course-meta-total-enrolled">
								<span class="meta-label"><?php esc_html_e('Students:', 'edumall-child'); ?></span>
						...
						<div class="tutor-mycourses-stats">
							...
							<div class="tutor-iconic-btn-group tutor-mr-n8">
								<a href="<?php echo esc_url(tutor_utils()->course_edit_link($post->ID)); ?>" class="tutor-iconic-btn tutor-my-course-edit">
									<i class="fal fa-pencil-alt" area-hidden="true"></i><?php esc_html_e('Edit', 'edumall-child'); ?>
								</a>
									...
									<div id="table-dashboard-course-list-<?php echo esc_attr($post->ID); ?>" class="tutor-dropdown tutor-dropdown-dark tutor-text-left">
											...
											<a class="tutor-dropdown-item" href="?<?php echo esc_attr($params); ?>">
												...
												<span>
													...
													if ($can_publish_course) {
														esc_html_e('Publish', 'edumall-child');
													} else {
														esc_html_e('Submit', 'edumall-child');
													}
													?>
												</span>
											</a>
										...
										<?php if (tutor()->has_pro && in_array($post->post_status, array(
											...
											<a class="tutor-dropdown-item" href="?<?php echo esc_attr($params); ?>">
												...
												<span><?php esc_html_e('Duplicate', 'edumall-child'); ?></span>
											</a>
										<?php endif; ?>
										<!-- # Duplicate Action -->
										...
										<?php if (tutor()->has_pro && in_array($post->post_status, array(CourseModel::STATUS_PUBLISH))) : ?>
											...
											<a class="tutor-dropdown-item" href="?<?php echo esc_attr($params); ?>">
												...
												<span><?php esc_html_e('Move to Draft', 'edumall-child'); ?></span>
											</a>
										<?php endif; ?>
										...
										<?php if (tutor()->has_pro && in_array($post->post_status, array(CourseModel::STATUS_PENDING))) : ?>
											...
											<a href="?<?php echo esc_attr($params); ?>" class="tutor-dropdown-item">
												...
												<span><?php esc_html_e('Cancel Submission', 'edumall-child'); ?></span>
											</a>
										<?php endif; ?>
										<!-- # Cancel Submission -->
										...
										<?php if (in_array($post->post_status, array(
											...
											<a href="#" data-tutor-modal-target="<?php echo esc_attr($id_string_delete); ?>" class="tutor-dropdown-item tutor-admin-course-delete">
												...
												<span><?php esc_html_e('Delete', 'edumall-child'); ?></span>
											</a>
										<?php endif; ?>
										<!-- # Delete Action -->
	...
	<?php else : ?>
		<div class="dashboard-no-content-found">
			<?php esc_html_e('You do not have any courses yet.', 'edumall-child'); ?>
		</div>
	<?php endif; ?>

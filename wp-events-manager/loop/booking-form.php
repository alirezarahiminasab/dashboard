	...
	<form name="event_register" class="event_register" method="POST">
		...
		<div class="row row-xs-center">
			...
			<?php else : ?>
				<div class="col-md-6">
					<div class="event_auth_form_field">
						<label for="event_register_qty"><?php esc_html_e('Quantity', 'edumall-child'); ?></label>
						<div class="quantity">
							<button type="button" class="increase"></button>
							<input type="number" name="qty" value="1" min="1" max="<?php echo esc_attr($event->get_slot_available()); ?>" id="event_register_qty" class="qty" />
			...
			<div class="<?php echo esc_attr($col_class); ?>">
					...
					if ($payments) { ?>
						<ul class="event_auth_payment_methods">
							...
							<?php foreach ($payments as $id => $payment) : ?>
								<li>
									<input id="payment_method_<?php echo esc_attr($id) ?>" type="radio" name="payment_method" value="<?php echo esc_attr($id) ?>" <?php if ($i === 0) {
																																											echo ' checked';
																																										} ?> />
				...
				<?php } else {
						wpems_print_notice('error', esc_html__('There are no payment gateway available. Please contact administrator to setup it.', 'edumall-child'));
					}
				} ?>
		...
		<div class="event_register_foot">
			...
			<button class="event_register_submit event_auth_button button" <?php if (!isset($payments)) {
																				echo 'disabled="disabled"';
																			} ?>>
				<?php esc_html_e('Book Now', 'edumall-child'); ?>
			</button>
		</div>

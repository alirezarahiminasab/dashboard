	...
	<ul class="entry-event-info">
		<li class="meta-price">
			<span class="meta-label">
				...
				<?php esc_html_e('Cost:', 'edumall-child'); ?>
			</span>
			<span class="meta-value">
				<span class="event-price"><?php printf('%s', $event->is_free() ? esc_html__('Free', 'edumall-child') : wpems_format_price($event->get_price())) ?></span>
			</span>
		</li>
		<li class="total">
			<span class="meta-label">
				<i class="meta-icon far fa-user-friends"></i>
				<?php esc_html_e('Total Slot:', 'edumall-child'); ?>
			</span>
		...
		<li class="booking_slot">
			<span class="meta-label">
				...
				<?php esc_html_e('Booked Slot:', 'edumall-child'); ?>
			</span>
	...
	<?php if ($can_register) : ?>
		<?php if (is_user_logged_in()) { ?>
			...
			if ($registered_time && 'once' === wpems_get_option('email_register_times') && $event->is_free()) { ?>
				<p class="event-register-message"><?php esc_html_e('You have registered this event before.', 'edumall-child'); ?></p>
			<?php } else { ?>
				<a class="event_register_submit event_auth_button event-load-booking-form button" data-event="<?php echo esc_attr(get_the_ID()) ?>"><?php esc_html_e('Book Now', 'edumall-child'); ?></a>
			<?php } ?>
		<?php } else { ?>
			<?php
			Edumall_Templates::render_button([
				...
				'text'        => esc_html__('Book Now', 'edumall-child'),
			...
			<p class="event-register-message">
				<?php echo sprintf(
					esc_html__('You must %1$slogin%2$s before register event.', 'edumall-child'),
		...
		<?php } ?>
	<?php else : ?>
		<p class="tp-event-notice error"><?php echo esc_html__('This event has expired', 'edumall-child'); ?></p>
	<?php endif; ?>

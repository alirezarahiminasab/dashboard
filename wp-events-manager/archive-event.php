				...
				<div class="archive-filter-bars row row-xs-center">
					<div class="archive-filter-bar archive-filter-bar-left col-md-6">
						<div class="archive-result-count">
							...
							$result_count_html = sprintf(_n('%s event', '%s events', $result_count, 'edumall-child'), '<span class="count">' . number_format_i18n($result_count) . '</span>');
							printf(
								wp_kses(
					...
					<div class="archive-filter-bar archive-filter-bar-right col-md-6">
						<div class="inner">
							<form id="archive-form-filtering" class="archive-form-filtering event-form-filtering" method="get">
								...
								$select_settings = [
									'fieldLabel' => esc_html__('Event Type:', 'edumall-child'),
								];
								?>
								<select class="edumall-nice-select event-type" name="filter_type" data-select="<?php echo esc_attr(wp_json_encode($select_settings)); ?>">

...
<?php if (empty($_REQUEST['checkemail'])) : ?>
	...
	<form name="forgot-password" class="forgot-password event-auth-form" action="" method="post">
		...
		<p class="form-row required">
			<label for="user_login"><?php esc_html_e('Username or Email:', 'edumall-child') ?>
		...
		<p class="form-row submit">
			<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e('Get New Password', 'edumall-child'); ?>" />
		</p>
	...
	<div class="event_auth_lost_pass_footer">
		<a href="<?php echo esc_attr(wpems_login_url()) ?>">
			<?php esc_html_e('Login', 'edumall-child'); ?>
		</a> |
		<?php if (!is_user_logged_in()) : ?>

			<a href="<?php echo esc_attr(wpems_register_url()) ?>">
				<?php esc_html_e('Create new user', 'edumall-child'); ?>
			</a>

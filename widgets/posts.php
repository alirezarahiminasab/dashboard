...
if (!class_exists('Edumall_WP_Widget_Posts')) {
	class Edumall_WP_Widget_Posts extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Posts', 'edumall-child');
			$this->widget_description = esc_html__('Get list blog post.', 'edumall-child');
			$this->settings           = array(
				'title'           => array(
					'type'  => 'text',
					'std'   => '',
		...
		{
			$cat_options = array(
				'recent_posts' => esc_html__('Recent Posts', 'edumall-child'),
				'sticky_posts' => esc_html__('Sticky Posts', 'edumall-child'),
			);
			$categories  = get_categories('hide_empty=0');
			if ($categories) {
				foreach ($categories as $category) {
					$cat_options[$category->term_id] = esc_html__('Category: ', 'edumall-child') . $category->name;
				}
			}

...
if (!class_exists('Edumall_WP_Widget_Product_Layered_Nav')) {
	class Edumall_WP_Widget_Product_Layered_Nav extends Edumall_WC_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Product Attribute Layered Nav', 'edumall-child');
			$this->widget_description = esc_html__('Display a list of attributes to filter products in your store.', 'edumall-child');
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__('Filter by', 'edumall-child'),
					'label' => esc_html__('Title', 'edumall-child'),
				),
				'attribute'    => array(
					'type'    => 'select',
					'std'     => '',
					'label'   => esc_html__('Attribute', 'edumall-child'),
					'options' => [],
				),
				'query_type'   => array(
					'type'    => 'select',
					'std'     => 'and',
					'label'   => esc_html__('Query type', 'edumall-child'),
					'options' => array(
						'and' => esc_html__('AND', 'edumall-child'),
						'or'  => esc_html__('OR', 'edumall-child'),
					),
				),
				'display_type' => array(
					'type'    => 'select',
					'std'     => 'list',
					'label'   => esc_html__('Display type', 'edumall-child'),
					'options' => array(
						'list'       => esc_html__('List', 'edumall-child'),
						'check-list' => esc_html__('Check List', 'edumall-child'),
						'inline'     => esc_html__('Inline', 'edumall-child'),
						'dropdown'   => esc_html__('Dropdown', 'edumall-child'),
					),
				),
				'labels'       => array(
					'type'    => 'select',
					'std'     => 'on',
					'label'   => esc_html__('Show labels', 'edumall-child'),
					'options' => array(
						'on'  => esc_html__('ON', 'edumall-child'),
						'off' => esc_html__('OFF', 'edumall-child'),
					),
				),
				'items_count'  => array(
					'type'    => 'select',
					'std'     => 'on',
					'label'   => esc_html__('Show items count', 'edumall-child'),
					'options' => array(
						'on'  => esc_html__('ON', 'edumall-child'),
						'off' => esc_html__('OFF', 'edumall-child'),
					),
		...
		protected function layered_nav_dropdown($terms, $taxonomy, $query_type)
		{
			...
			if ($taxonomy !== $this->get_current_taxonomy()) {
				...
				$any_label            = apply_filters(
					...
					sprintf(esc_html__('Any %s', 'edumall-child'), $taxonomy_label),
					$taxonomy_label,
					$taxonomy
				);

				echo '<a href="#" class="filter-pseudo-link link-taxonomy-' . $taxonomy_filter_name . '">' . esc_html__(
					'Apply filter',
					'edumall-child'
				) . '</a>';

				echo '<select class="dropdown_layered_nav_' . $taxonomy_filter_name . '" data-filter-url="' . preg_replace(
					'%\/page\/[0-9]+%',
					'',
					str_replace(
						array(

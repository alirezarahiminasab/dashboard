...
if (!class_exists('Edumall_WP_Widget_Product_Price_Filter')) {
	class Edumall_WP_Widget_Product_Price_Filter extends Edumall_WC_Widget_Base
	{
		...
		{
			$this->widget_id          = 'edumall-wp-widget-product-price-filter';
			$this->widget_cssclass    = 'edumall-wp-widget-product-price-filter';
			$this->widget_name        = esc_html__('[Edumall] Product Price Filter', 'edumall-child');
			$this->widget_description = esc_html__('Display a price range list to filter products in your store.', 'edumall-child');
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__('Price Filter', 'edumall-child'),
		...
		public function widget($args, $instance)
		{
			...
			if (!empty($links)) {
?>
				<div class="edumall-product-price-filter">
					<ul>
...
<?php
		...
		private function generate_price_links($min, $max, $min_price, $max_price)
		{
			...
			$steps = apply_filters('edumall_price_filter_steps', 5);

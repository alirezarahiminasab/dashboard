...
if (!class_exists('Edumall_WP_Widget_Product_Badge')) {
	class Edumall_WP_Widget_Product_Badge extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Product Badge', 'edumall-child');
			$this->widget_description = esc_html__('Get List Badge', 'edumall-child');
			$this->settings           = array(
				'title'              => array(
					'type'  => 'text',
					'std'   => '',
				...
				'show_badge_hot'     => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('Featured product', 'edumall-child'),
				),
				'badge_hot_text'     => array(
					'type'  => 'text',
					'std'   => esc_html__('Hot items', 'edumall-child'),
					'label' => esc_html__('Hot Badge Text', 'edumall-child'),
				),
				'show_badge_on_sale' => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('On sale product', 'edumall-child'),
				),
				'badge_sale_text'    => array(
					'type'  => 'text',
					'std'   => esc_html__('Sale', 'edumall-child'),
					'label' => esc_html__('On sale Badge Text', 'edumall-child'),
				),
				'show_badge_new'     => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('New product', 'edumall-child'),
				),
				'badge_new_text'     => array(
					'type'  => 'text',
					'std'   => esc_html__('New Arrivals', 'edumall-child'),
					'label' => esc_html__('New Badge Text', 'edumall-child'),
				),
			);

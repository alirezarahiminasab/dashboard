...
if (!class_exists('Edumall_WP_Widget_Product_Banner')) {
	class Edumall_WP_Widget_Product_Banner extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Product Banner', 'edumall-child');
			$this->widget_description = esc_html__('Product Banner', 'edumall-child');
			$this->settings           = array(
				'title'         => array(
					'type'  => 'text',
					'std'   => '',
				...
				'product_id'    => array(
					'type'  => 'text',
					'std'   => '',
					'label' => esc_html__('Product ID', 'edumall-child'),
				),
				'background'    => array(
					'type'  => 'image',
					'std'   => '',
				...
				'show_category' => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('Show Category', 'edumall-child'),
				),
			);
...
?>
			<?php if ($product) : ?>
				...
				<div class="banner-product-wrapper" <?php if (!empty($background_url)) : ?> style="background-image: url( <?php echo esc_url($background_url); ?> )" <?php endif; ?>>
						...
						<?php if ($product->is_on_sale()) : ?>
							<div class="product-banner-badge">
								...
								if ($product->is_type('simple') || $product->is_type('external')) {
									...
									echo '<span class="badge-value">' . "{$percentage}%" . '</span><span class="badge-text">' . esc_html__('Off', 'edumall-child') . '</span>';
								} else {
									echo '<span class="badge-value">' . esc_html__('Sale !', 'edumall-child') . '</span>';
								}
								?>
							</div>
						<?php endif; ?>
					...
					<?php \Edumall_Templates::render_button([
						'text'          => esc_html__('Shop now', 'edumall-child'),
						'link'          => [

...
if (!class_exists('Edumall_WP_Widget_Events')) {
	class Edumall_WP_Widget_Events extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Events', 'edumall-child');
			$this->widget_description = esc_html__('Get list events.', 'edumall-child');
			$this->settings           = array(
				'title'           => array(
					'type'  => 'text',
					'std'   => esc_html__('Recent Events', 'edumall-child'),
					'label' => esc_html__('Title', 'edumall-child'),
				),
				'filter_by'       => array(
					'type'    => 'select',
					'std'     => 'recent',
					'label'   => esc_html__('Filter By', 'edumall-child'),
					'options' => [],
				),
				'num'             => array(
					'type'  => 'number',
				...
				'show_price'      => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('Show Price', 'edumall-child'),
				),
				'show_categories' => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show Categories', 'edumall-child'),
				),
				'show_date'       => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('Show Date', 'edumall-child'),
				),
			);
		...
		public function set_form_settings()
		{
			$filter_by_options = array(
				'recent'  => esc_html__('Recent Events', 'edumall-child'),
				'related' => esc_html__('Related Events', 'edumall-child'),
				'popular' => esc_html__('Popular Events', 'edumall-child'),
			);
			$terms             = get_terms([
			...
			if (!empty($terms) && !is_wp_error($terms)) {
				foreach ($terms as $term) {
					$filter_by_options[$term->term_id] = esc_html__('Category: ', 'edumall-child') . $term->name;
				}
			}
							...
							<div class="event-info">
								...
								<?php if ($show_price) : ?>
									<div class="event-price price">
										<?php printf('%s', $event->is_free() ? esc_html__('Free', 'edumall-child') : wpems_format_price($event->get_price())); ?>
									</div>
								<?php endif; ?>

...
if (!class_exists('Edumall_WP_Widget_Event_Filtering')) {
	class Edumall_WP_Widget_Event_Filtering extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Event Filtering', 'edumall-child');
			$this->widget_description = esc_html__('Display a form to filter events with some criteria.', 'edumall-child');
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__('Find events', 'edumall-child'),
			...
			wp_register_script('edumall-event-filtering', EDUMALL_THEME_ASSETS_URI . "/js/events-manager/event-filtering{$min}.js", ['jquery-ui-datepicker'], EDUMALL_THEME_VERSION, true);
		...
		public function widget($args, $instance)
		{
			...
			$category_dropdown_args = array(
				...
				'show_count'      => 0,
				'hierarchical'    => 1,
				'show_option_all' => esc_html__('All categories', 'edumall-child'),
...
?>
			<form method="get" action="<?php echo esc_url($form_action); ?>" class="event-filtering-form">
				<div class="form-group">
					<input type="text" id="filter_start_date" class="form-input-date readonly" autocomplete="off" readonly placeholder="<?php echo esc_attr_x('Event from', 'placeholder', 'edumall-child'); ?>" value="<?php echo esc_attr($current_start_date); ?>" name="filter_start_date" />
				...
				<div class="form-group">
					<input type="text" id="filter_location" class="search-field" placeholder="<?php echo esc_attr_x('Location', 'placeholder', 'edumall-child'); ?>" value="<?php echo esc_attr($current_location); ?>" name="filter_location" />
					<span class="form-icon">
				...
				<div class="form-group">
					<input type="search" class="search-field" placeholder="<?php echo esc_attr_x('Keyword&hellip;', 'placeholder', 'edumall-child'); ?>" value="<?php echo esc_attr($current_name); ?>" name="filter_name" />
					<span class="form-icon">
				...
				<div class="form-submit">
					<input type="hidden" value="<?php echo esc_attr($theme_preset); ?>" name="event_archive_preset">
					<input type="hidden" value="<?php echo esc_attr($current_type); ?>" name="filter_type">
					<button type="submit" class="search-submit">
						<span class="search-btn-text">
							<?php echo esc_html_x('Find events', 'submit button', 'edumall-child'); ?>
						</span>
					</button>
				</div>
			</form>

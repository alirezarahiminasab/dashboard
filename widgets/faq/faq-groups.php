...
if (!class_exists('Edumall_WP_Widget_FAQ_Group')) {
	class Edumall_WP_Widget_FAQ_Group extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] FAQ Groups', 'edumall-child');
			$this->widget_description = esc_html__('A list or dropdown of FAQ group.', 'edumall-child');
			$this->settings           = array(
				'title'              => array(
					'type'  => 'text',
					'std'   => esc_html__('FAQ Groups', 'edumall-child'),
				...
				'source'             => array(
					...
					'options' => array(
						...
						'children'     => esc_html__('Only show children of the current group', 'edumall-child'),
						'top_children' => esc_html__('Only show top group\'s children of the current group', 'edumall-child'),
					),
				),
				'orderby'            => array(
					'type'    => 'select',
					...
					'options' => array(
						'order' => esc_html__('Group order', 'edumall-child'),
				...
				'dropdown'           => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show as dropdown', 'edumall-child'),
				),
				'count'              => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show post counts', 'edumall-child'),
				),
				'hierarchical'       => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__('Show hierarchy', 'edumall-child'),
				),
				'hide_empty'         => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Hide empty groups', 'edumall-child'),
				),
				'max_depth'          => array(
					'type'  => 'text',
					'std'   => '',
					'label' => esc_html__('Maximum depth', 'edumall-child'),
					'desc'  => esc_html__('For eg: Input 1 to show only top groups.', 'edumall-child'),
				),
		...
		public function widget($args, $instance)
		{
				...
				if ( jQuery().selectWoo ) {
					var faq_group_select = function() {
						jQuery( '.dropdown-faq-group' ).selectWoo( {
							placeholder: '" . esc_js(__('Select a group', 'edumall-child')) . "',
							minimumResultsForSearch: 5,
							width: '100%',
							allowClear: true,
							language: {
								noResults: function() {
									return '" . esc_js(_x('No matches found', 'enhanced select', 'edumall-child')) . "';
								}
							}
						} );
					};
			...
			"
				...
				$list_args['show_option_none']           = esc_html__('No groups exist.', 'edumall-child');

...
if (!class_exists('Edumall_WP_Widget_FAQs')) {
	class Edumall_WP_Widget_FAQs extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] FAQs', 'edumall-child');
			$this->widget_description = esc_html__('Get list faq post.', 'edumall-child');
			$this->settings           = array(
				'title'      => array(
					'type'  => 'text',
					'std'   => '',
				...
				'show_group' => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show Group', 'edumall-child'),
				),
				'show_date'  => array(
					'type'  => 'checkbox',
					'std'   => 0,
		...
		public function set_form_settings()
		{
			$filter_by_options = array(
				'recent'  => esc_html__('Recent FAQs', 'edumall-child'),
				'related' => esc_html__('Related FAQs', 'edumall-child'),
			);
			$terms             = get_terms([
			...
			if (!empty($terms) && !is_wp_error($terms)) {
				foreach ($terms as $term) {
					$filter_by_options[$term->term_id] = sprintf(__('Group: %s', 'edumall-child'), $term->name); // XSS OK.
				}
			}

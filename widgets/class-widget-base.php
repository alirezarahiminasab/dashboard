...
/**
 * Abstract Class: Widget Base
 *
 * @version  1.0
 * @extends  WP_Widget
 */
if (!class_exists('Edumall_WP_Widget_Base')) {
	abstract class Edumall_WP_Widget_Base extends WP_Widget
	{
...
?>
					...
					?>
						<div class="kungfu-attach-wrap">
							...
							<input type="hidden" class="kungfu-media" name="<?php echo esc_attr($this->get_field_name($key)); ?>" value="<?php echo esc_attr($value); ?>" />
							<a class="kungfu-media-open kungfu-button success">
								<i class="fa fa-upload"></i><?php esc_html_e('Upload', 'edumall-child'); ?>
							</a>
							<a class="kungfu-media-remove kungfu-button danger" <?php if (empty($value)) : ?> style="display:none" <?php endif; ?>><i class="fa fa-trash-o"></i><?php esc_html_e('Remove', 'edumall-child'); ?></a>
						</div>
<?php
						break;
					...
					default:
						do_action('edumall_widget_field_' . $setting['type'], $key, $value, $setting, $instance);
						break;
				}
			}
		}

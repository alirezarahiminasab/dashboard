...
if (!class_exists('Edumall_WP_Widget_Course_Rating_Filter')) {
	class Edumall_WP_Widget_Course_Rating_Filter extends Edumall_Course_Layered_Nav_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Course Rating Filter', 'edumall-child');
			$this->widget_description = esc_html__('Shows rating in a widget which lets you narrow down the list of courses when viewing courses.', 'edumall-child');
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__('Filter by rating', 'edumall-child'),
				...
				'hide_empty'   => array(
					'type'    => 'select',
					'std'     => 'off',
					'label'   => esc_html__('Hide empty items', 'edumall-child'),
					'options' => array(
						'on'  => esc_html__('Yes', 'edumall-child'),
						'off' => esc_html__('No', 'edumall-child'),
					),
				),
			);

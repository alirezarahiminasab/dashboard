...
if (!class_exists('Edumall_WP_Widget_Course_Categories')) {
	class Edumall_WP_Widget_Course_Categories extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Course Categories', 'edumall-child');
			$this->widget_description = esc_html__('A list or dropdown of course categories.', 'edumall-child');
			$this->settings           = array(
				'title'              => array(
					'type'  => 'text',
					'std'   => esc_html__('Course categories', 'edumall-child'),
				...
				'orderby'            => array(
					...
					'options' => array(
						'order' => esc_html__('Category order', 'edumall-child'),
				...
				'count'              => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show course counts', 'edumall-child'),
				),
				'hierarchical'       => array(
					'type'  => 'checkbox',
					'std'   => 1,
				...
				'show_children_only' => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Only show children of the current category', 'edumall-child'),
				),
				'hide_empty'         => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Hide empty categories', 'edumall-child'),
				),
				'max_depth'          => array(
					'type'  => 'text',
					'std'   => '',
					'label' => esc_html__('Maximum depth', 'edumall-child'),
					'desc'  => esc_html__('For eg: Input 1 to show only top categories.', 'edumall-child'),
				),
			);
		...
		public function widget($args, $instance)
		{
				...
				if ( jQuery().selectWoo ) {
					var tutor_course_category_select = function() {
						jQuery( '.dropdown-course-category' ).selectWoo( {
							placeholder: '" . esc_js(__('Select a category', 'edumall-child')) . "',
			...
			"
				...
				$list_args['show_option_none']           = esc_html__('No categories exist.', 'edumall-child');

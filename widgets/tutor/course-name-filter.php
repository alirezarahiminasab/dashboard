...
if (!class_exists('Edumall_WP_Widget_Course_Name_Filter')) {
	class Edumall_WP_Widget_Course_Name_Filter extends Edumall_Course_Layered_Nav_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Course Name Filter', 'edumall-child');
			$this->widget_description = esc_html__('Show form to narrow courses by course name.', 'edumall-child');
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__('Search', 'edumall-child'),
...
?>
			<form role="search" method="get" class="search-form" action="<?php echo esc_url($base_link); ?>">
				<label class="screen-reader-text" for="course-search-field"><?php esc_html_e('Search for:', 'edumall-child'); ?></label>
				<input type="search" id="course-search-field" class="search-field" placeholder="<?php echo esc_attr_x('Find your course', 'placeholder', 'edumall-child'); ?>" value="<?php echo esc_attr($current_value); ?>" name="<?php echo esc_attr($filter_by_key); ?>" />

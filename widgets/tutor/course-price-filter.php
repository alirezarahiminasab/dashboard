...
if (!class_exists('Edumall_WP_Widget_Course_Price_Filter')) {
	class Edumall_WP_Widget_Course_Price_Filter extends Edumall_Course_Layered_Nav_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Course Price Filter', 'edumall-child');
			$this->widget_description = esc_html__('Shows prices in a widget which lets you narrow down the list of courses when viewing courses.', 'edumall-child');
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__('Filter by price', 'edumall-child'),
		...
		protected function layered_nav_list($instance)
		{
			...
			$price_options = [
				...
				'free' => esc_html__('Free', 'edumall-child'),
				'paid' => esc_html__('Paid', 'edumall-child'),
			];

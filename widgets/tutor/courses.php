...
if (!class_exists('Edumall_WP_Widget_Courses')) {
	class Edumall_WP_Widget_Courses extends Edumall_WP_Widget_Base
	{
		...
		public function __construct()
		{
			...
			$this->widget_name        = esc_html__('[Edumall] Courses', 'edumall-child');
			$this->widget_description = esc_html__('Get list courses.', 'edumall-child');
			$this->settings           = array(
				'title'           => array(
					'type'  => 'text',
					'std'   => esc_html__('Recent Courses', 'edumall-child'),
				...
				'show_badge'      => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__('Show Badge', 'edumall-child'),
				),
			);
		...
		public function set_form_settings()
		{
			$filter_by_options = array(
				'recent'  => esc_html__('Recent Courses', 'edumall-child'),
				'related' => esc_html__('Related Courses', 'edumall-child'),
			);
			$terms             = get_terms([
			...
			if (!empty($terms) && !is_wp_error($terms)) {
				foreach ($terms as $term) {
					$filter_by_options[$term->term_id] = sprintf(__('Category: %s', 'edumall-child'), $term->name); // XSS OK.
				}
			}

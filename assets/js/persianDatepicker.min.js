...
(function ($) {
  ...
  var persianDatepicker = (function () {
    function persianDatepicker(element, userOptions) {
      ...
      self.cellStyle =
        "style='width:" + _cw + "px; font-size:" + _fontSize + "px; ' ";
      self.headerStyle = "style='font-size:" + (_fontSize + 4) + "px;' ";
      self.selectUlStyle =
        "style='margin-top:" +
        _ch +
        "px;height:" +
      ...
      if (
        self.options.selectedDate == undefined &&
        !self.options.showGregorianDate
      ) {
      ...
      self._persianDate =
        self.options.selectedDate != undefined
          ? new persianDate().parse(self.options.selectedDate)
          : self.now();
      if (
        options.selectableYears != undefined &&
        options.selectableYears._indexOf(self._persianDate.year) == -1
      )
      ...
      self.calendar = $(
        '<div id="' + self._id + '" class="pdp-' + options.theme + '" />',
      );

      if (self.options.startDate != null) {
        self.options.selectableYears = [];
        for (
          var i = self.persianDate.parse(self.options.startDate).year;
          i <= self.persianDate.parse(self.options.endDate).year;
          i++
        )
      ...
      if (options.selectedBefore) {
        if (self.options.selectedDate != undefined) {
          ...
          self.showDate(
            el,
            self.persianDate
              .parse(self.options.selectedDate)
              .toString(
                "YYYY/MM/DD/" +
                  self.jDateFunctions.getWeekday(
                    self.persianDate.parse(self.options.selectedDate),
                  ),
                self.now().gDate,
                options.showGregorianDate,
              ),
          );
        } else {
          ...
          self.showDate(
            ...
            self
              .now()
              .toString(
                "YYYY/MM/DD/" + self.jDateFunctions.getWeekday(self.now()),
              ),
            self.now().gDate,
            options.showGregorianDate,
          );
    ...
    persianDatepicker.prototype = {
      ...
      header: function () {
        ...
        if (
          ...
          self.persianDate.parse(self.options.endDate).year >
            self.persianDate.year ||
          self.persianDate.parse(self.options.endDate).month >
            self.persianDate.month
        ) {
          _next.on("click", function () {
            nextMonth = self.persianDate.month + 1;
            for (
              ;
              self.options.selectableMonths._indexOf(nextMonth) == -1 &&
              nextMonth < 13;
              nextMonth++
            );
        ...
        var _monthSelect = $(
          '<ul class="monthSelect" ' + self.selectUlStyle + " />",
        ).hide();
        var _yearSelect = $(
          '<ul class="yearSelect" ' + self.selectUlStyle + " />",
        ).hide();
        // Build month label
        var _monthText = $("<span/>")
        ...
        var _yearText = $("<span/>")
          .html(
            self.options.persianNumbers
              ? self.jDateFunctions.toPersianNums(self.persianDate.year)
              : self.persianDate.year,
          )
          .on("mousedown", function () {
            return false;
        ...
        _startDate =
          self.options.startDate != null
            ? self.persianDate.parse(self.options.startDate)
            : self.persianDate.parse("1/1/1");
        _endDate =
          self.options.endDate != null
            ? self.persianDate.parse(self.options.endDate)
            : self.persianDate.parse("9999/1/1");
        ...
        var getSelectableYears = function (f, l) {
          ...
          var arr = [];
          for (i = b; i < a && b > 0; i++) arr.push(parseInt(i));
          $.each(
            self.options.selectableYears || (pre ? arr.reverse() : arr),
            function (i, v) {
              var o = $("<li " + self.selectYearLiStyle + " />").html(
                self.options.persianNumbers
                  ? self.jDateFunctions.toPersianNums(v)
                  : v,
              );
        ...
        for (i = 1; i <= 12; i++) {
          ...
          var o =
            self.options.selectableMonths._indexOf(i) == -1 ||
            (_startDate.year == self.persianDate.year &&
              _startDate.month > i) ||
            (_endDate.year == self.persianDate.year && i > _endDate.month)
              ? $(
                  '<li class="disableMonth" ' + self.selectMonthLiStyle + " />",
                ).html(m)
        ...
        // selectable years
        _yearSelect.on("scroll", function () {
          if (self.options.selectableYears == undefined) {
            ...
            if (
              $(this).scrollTop() < _com.toString().length &&
              firstYear >= 30
            ) {
              $(this).scrollTop(_com.toString().length * 100);
            }
        ...
        if (
          self.options.startDate == null ||
          self.persianDate.parse(self.options.startDate).year <
            self.persianDate.year ||
          self.persianDate.parse(self.options.startDate).month <
            self.persianDate.month
        ) {
      ...
      dows: function () {
        _row = $('<div class="dows" />');
        for (i = 0; i < 7; i++) {
          _cell = $('<div class="dow cell " ' + this.cellStyle + " />").html(
        ...
        for (var row = 0, cellIndex = 0; row < 5 + 1; row++) {
          ...
          for (var col = 0; col < 7; col++, cellIndex++) {
            if (cellIndex < _start || cellIndex - _start + 1 > _end) {
              _cell = $(
                '<div class="nul cell " ' + self.cellStyle + " />",
              ).html("&nbsp;");
            } else {
              ...
              if (
                self.options.startDate != null &&
                (self.persianDate.parse(self.options.startDate).compare(_dt) ==
                  -1 ||
                  self.persianDate.parse(self.options.endDate).compare(_dt) ==
                    1)
              )
                _disday = "disday";
              ...
              if (self.options.selectedDate != undefined) {
                if (
                  self.persianDate.parse(self.options.selectedDate).date ==
                  cellIndex - _start + 1
                )
                  _selday = "selday";
              ...
              _cell.attr(
                "data-gdate",
                self.jDateFunctions.getGDate(_dt)._toString("YYYY/MM/DD"),
              );
              _cell.html(
                self.options.persianNumbers
                  ? self.jDateFunctions.toPersianNums(cellIndex - _start + 1)
                  : cellIndex - _start + 1,
              );

              if (
                self.options.startDate == undefined ||
                (self.persianDate.parse(self.options.startDate).compare(_dt) !=
                  -1 &&
                  self.persianDate.parse(self.options.endDate).compare(_dt) !=
                    1)
              )
                _cell.on("click", function () {
      ...
      footer: function () {
        ...
        if (
          self.options.selectableMonths._indexOf(self.persianDate.month) > -1
        ) {
      ...
      showDate: function (el, jDate, gDate, showGdate) {
        ...
        if (el.is("input:text")) {
          if (showGdate) el.val(gDate);
          else el.val(jDate);
        } else {
          if (showGdate) el.html(gDate);
          else el.html(jDate);
        }
  ...
  (function () {
    ...
    Date.prototype._toString = function (formatDate) {
      ...
      return formatDate
        ...
        .replace(
          "hh",
          this.getHours() == 0 ? new Date().getHours() : this.getHours(),
        )
        .replace(
          "mm",
          this.getMinutes() == 0 ? new Date().getMinutes() : this.getMinutes(),
        )
        .replace(
          "ss",
          this.getSeconds() == 0 ? new Date().getSeconds() : this.getSeconds(),
        )
        .replace(
          "0h",
  ...
  persianDate.prototype = {
    ...
    addDay: function (d) {
      ...
      for (var i = 0; i < to; i++) {
        ...
        r = r.addMonth(-1);
        var lastDayOfMonth =
          d > 0 ? jdf.getLastDayOfMonth(this) : jdf.getLastDayOfMonth(r);
    ...
    parse: function (s) {
      ...
      (r.year = parseInt(y)),
        (r.month = parseInt(m)),
        (r.date = parseInt(d)),
        (r.day = jdf.getWeekday(r)),
        (r.gDate = jdf.jalali_to_gregorian(r));
      return r;
    },
    toString: function (formatDate) {
      ...
      return formatDate
        ...
        .replace(
          "0h",
          this.gDate.getHours() > 9
            ? this.gDate.getHours()
            : "0" + this.gDate.getHours(),
        )
        .replace(
          "0m",
          this.gDate.getMinutes() > 9
            ? this.gDate.getMinutes()
            : "0" + this.gDate.getMinutes(),
        )
        .replace(
          "0s",
          this.gDate.getSeconds() > 9
            ? this.gDate.getSeconds()
            : "0" + this.gDate.getSeconds(),
        )
        .replace(
          "tm",
...
var jDateFunctions = (function () {
  ...
  jDateFunctions.prototype = {
    toPersianNums: function (s) {
      ...
      res = "";
      for (i = 0; i < strnum.length; i++) res += nums[parseInt(strnum[i])];
      return res;
    },
